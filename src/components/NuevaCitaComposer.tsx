...
/**
 * NuevaCitaComposer — núcleo reusable del flujo "Nueva cita" (rediseño). Una sola
 * pantalla basada en servicios: con 1 servicio es una cita normal, con 2+ son
 * procedimientos consecutivos de una visita (sin toggle de modo). Toda la lógica/
 * datos viven en useAppointmentComposer; acá solo el layout app-shell 2-col +
 * footer sticky. Sin `<MainLayout>` propio — lo envuelve quien lo use, ya sea la
 * página `/citas/nueva` o un Drawer embebido (ej. FAB de la agenda mensual).
 */
export default function NuevaCitaComposer({
  initialDate,
  onCreated,
}: {
  initialDate?: Date;
  ...
  const c = useAppointmentComposer(initialDate);
              ...
              <div className="mb-3 flex items-center justify-between gap-2">
                <div className="flex min-w-0 items-baseline gap-2">
                  <Label className="text-base font-semibold">
                    {c.isLoadingCatalog || showServices ? 'Servicios a realizar' : 'Duración'}
                  </Label>
...
function MobileDateTime({ composer: c }: { composer: Composer }) {
  ...
  return (
    <section className="lg:hidden min-w-0">
      {/* `nested`: este Drawer puede quedar dentro de OTRO Drawer full-screen (ej. el
          Drawer "Nueva cita" embebido en la agenda mensual). Sin `nested`, vaul
          restaura el scroll-lock del body al cerrar ESTE drawer aunque el exterior
          siga abierto (bug de Safari/iOS, variable global compartida entre todos los
          Drawer.Root de la página) — `nested` deja ese manejo 100% a cargo del
          exterior. Inofensivo cuando este componente vive en la página completa
          (`/citas/nueva`, sin drawer exterior). */}
