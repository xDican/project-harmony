...
- **Patrón Hub + Drawer (default para pantallas de catálogo/CRUD):** una sola pantalla persistente ("hub": grid o lista) que nunca navega fuera — crear/ver/editar cada ítem vive en una superposición (`Drawer` en mobile, `Dialog` en desktop), nunca en una ruta nueva. Ejemplos reales: `Calendario.tsx` (hub = grid mensual; FAB "+" abre `Drawer` con `NuevaCitaComposer` embebido) y `DayAppointmentsModal.tsx`/`NewScheduleExceptionModal.tsx` (tocar un ítem abre `Drawer`/`Dialog` con el detalle; modales anidados vía portals de Radix funcionan sin conflicto, ver "Reagendar" dentro de `DayAppointmentsModal`). **Regla de cierre explícito:** el `Drawer` (vaul) SIEMPRE con `dismissible={false}` + botón X visible en el header — vaul cierra por distancia O por velocidad (`VELOCITY_THRESHOLD=0.4` interno, no configurable), un swipe rápido cierra con mínimo arrastre pese a `closeThreshold`; sin `dismissible={false}` el swipe accidental es un riesgo real en mobile (fix real en `Calendario.tsx:199`, Drawer "Nueva cita"). Si el Drawer tiene un form con varios inputs, sumar también `repositionInputs={false}` (mismo archivo/línea) — por default vaul redimensiona el drawer en vivo cuando un input tiene foco y el teclado está abierto, pensado para bottom-sheets cortos, no para un form completo de `100dvh`.
