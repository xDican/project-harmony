...
- **`meta-webhook/index.ts`: 38 errores de TypeScript preexistentes** (`deno check`), presentes desde su creacion (commit `0c0523e`, 20 Feb 2026 — migracion Twilio→Meta Cloud API). Causa: `createClient()` en la linea ~1427 se llama sin pasarle el generico `Database` — sin ese tipo, ciertas llamadas `.insert()`/`.select()` resuelven el resultado como `never` (21× `TS2339 Property 'x' does not exist on type 'never'`, 9× `TS2345`, 4× `TS2769`, 4× `TS2322`). Probado tambien un choque de version entre `@supabase/supabase-js@2.39.0` (bot-handler) vs `2.39.3` (meta-webhook y su `_shared/*`) — al alinear bajo 2.39.0 los errores bajaron de 38 a 35, no a cero (revertido, no se dejo a medias) — confirma que el generico faltante es la causa dominante, no solo version. **No afecta runtime** (la funcion deploya y corre bien hace 5 meses; `deno check` es un gate propio, no un requisito de Supabase). Fix real: pasar `Database` (de `src/integrations/supabase/types.ts`) al `createClient` de este archivo, y auditar si otras edge functions tienen el mismo hueco. No urgente — hallazgo del 24 Jul al intentar typecheckear meta-webhook por primera vez (para el fix de silencio de Coexistence). **Ampliado 6 Ago:** `process-media-async/index.ts` tiene el mismo hueco (mismo sintoma, `createClient` sin `Database`) — confirmado con `deno check` (51 errores baseline, mismos con y sin el diff de hoy via `git stash`). Mismo fix real pendiente, mismo "no urgente".
- **Drift real entre historial de migraciones local y remoto** (hallazgo 6 Ago, `supabase migration list --linked`): decenas de timestamps que existen solo en local o solo en remoto (aplicadas via MCP `apply_migration` con otro nombre de archivo, o el archivo local nunca se pusheo formalmente). El schema real converge pero el tracking de `supabase_migrations.schema_migrations` no es confiable — **no correr `supabase db push` a ciegas**, puede intentar reaplicar algo bajo un ID distinto y romper. Mientras no se reconcilie: aplicar migraciones nuevas directo con `supabase db query --file <path>` (mismo efecto que MCP `apply_migration`, sin la capa de tracking) y dejar el archivo en el repo solo como historial. Nota aparte: **MCP Supabase dejo de autenticar de nuevo** (6 Ago, error "Unrecognized client_id" en el OAuth flow) — el workaround de la sesion de hoy fue usar `npx supabase` CLI directo (ya estaba logueado localmente), que cubre todo lo que hacia el MCP (`db query`, `functions deploy`, `secrets list`, etc.) sin necesitar el flow OAuth roto.

> Ultima actualizacion: 10 Ago 2026 (**PILOTO BIENES RAÍCES — BRUNO: feature completo, Fases 0-3 de Propiedades + Fases 0-3 de integración bot-handler, TODO cerrado, deployado y pusheado**. Ver checkpoint ▼ "10 Ago 2026" para el detalle completo. Primera entrada de OrionCare a un rubro no-médico. Resumen: (1) **Propiedades** — tabla nueva `properties` + `organizations.vertical` + hub/drawer UI (`PropertiesPage.tsx`/`PropertyDrawer.tsx`), QA-aprobado en vivo por Diego tras varias rondas de ajuste; de paso se extrajo `ResponsiveModal` (componente compartido nuevo) y se nombró el patrón "Hub + Drawer" (documentado en `CLAUDE.md`). (2) **Integración bot-handler** — `meta-webhook` matchea el código de propiedad (`referral` de Meta ads/posts, o texto plano `wa.me?text=COD-104`) y lo persiste en `conversations.interest_property_id` ANTES del debounce; `bot-handler` enruta por `organizations.vertical` hacia un flujo conversacional nuevo (`realestate.ts`, paralelo a `sdr.ts`, sin tocarlo — deliberadamente NO reusa `offerSlotsForDate`/`acceptSlot` porque toman un `ctx: SdrContext` médico-específico y sin `deno check` disponible no había forma segura de verificar un stub) que presenta la ficha real y agenda la visita reusando `createAppointmentWithPatient` (1 sola línea nueva: `property_id`). **Bug real encontrado y corregido antes de deployar:** `calendars` NO tiene columna `doctor_id` (verificado con `execute_sql` contra `information_schema.columns`) — la relación real es vía `calendar_doctors`/`whatsapp_line_doctors`, mismo mecanismo que ya usa `sdr.ts` para auto-asignar calendario. **Hallazgo que ahorró trabajo:** el motor de citas YA degrada solo a un path `'duration'` (selector de duración, sin catálogo de servicios) para cualquier org sin `service_types` — la org de Bruno cae ahí gratis, sin código nuevo, porque propiedades vive en su propia tabla. **Deploy de `bot-handler` confirmado por Diego vía AskUserQuestion antes de correr** (mismo criterio que toda la sesión: archivo más central del sistema, cada mensaje de cada org pasa por ahí) — logs post-deploy limpios, cero tráfico real todavía (org de Bruno no existe). Commits: `7f73465` (ResponsiveModal), `38c617a` (Propiedades), `88fd3f3` (integración bot-handler) — los 3 en `feat/pwa-minimo`, pusheados. `tsc --noEmit` y `npm run build` limpios; `npm run lint` tiene deuda preexistente en todo el repo (`no-explicit-any`), no es regresión de esta sesión. **Gap conocido, deliberadamente NO resuelto:** el nav de Bruno es aditivo (se agregó "Propiedades" sin ocultar "Pacientes"/vocabulario clínico) — parqueado en `rubro-organizacion-onboarding`, no bloquea el piloto porque Bruno opera solo y usa principalmente el bot de WhatsApp. **Pendiente semana del 17/8 (operativo, no código):** confirmar nombre real del negocio de Bruno, crear su organización (`vertical='bienes_raices'`) + su fila en `doctors` (prerrequisito: `appointments.doctor_id` es `NOT NULL`) + conectar su número — primera prueba end-to-end real recién ahí.)
> Update previo: 9 Ago 2026 (**SDR: "pregunta de interés" antes de invitar a agendar — implementada, QA en vivo con Diego, 2 bugs reales cazados y corregidos, CONFIRMADO funcionando**. Ver checkpoint ▼ "9 Ago 2026" para el detalle completo. Origen: Diego notó en el inbox de Hanoy que el SDR cierra cada respuesta con "¿le agendo?" en seco y el lead se queda callado; comparamos contra las 7 conversaciones reales del SDR desde su activación (27 Jul) + revisamos ~34% de sesiones del bot clásico (Wilmer/Medilaser/Consultorio Familiar) que mueren justo después de una FAQ. Patrón confirmado con datos: en las 4 conversaciones donde Hanoy intervino manualmente, las 4 veces hizo una pregunta cálida sobre el caso ANTES de cerrar — y las 2 que sí agendaron son justo esas; 3/7 leads del SDR (43%) quedaron muertos sin que nadie (bot ni humano) reintentara (backlogueado aparte, no se trabaja ahora). **Cambio implementado:** nuevo campo `interest_question_asked` en el contrato de salida del LLM (`_shared/sdr-prompt.ts`) + flag `session.context.sdrAskedInterestQuestion` (mismo patrón que `sdrBookingSetup`, sin migración) — el guión ahora permite UNA pregunta de interés genuina sobre el caso puntual del paciente antes de invitar a agendar, salvo que el paciente ya pida agendar directo (eso ya bypaseaba el `reply` vía la rama `wantsBooking`, gratis). **QA en vivo reveló 2 bugs reales:** (1) el bot abría con empatía enlatada ("entiendo lo incómodo que es eso") que Hanoy NUNCA usa en sus conversaciones reales — corregido, prohibido explícito en el prompt; (2) **bug de fondo pre-existente** (no introducido esta sesión, ya visto el 5 Ago con Alejandro): el SDR caía sistemáticamente al bot clásico en el 2do turno de CUALQUIER conversación — nunca se había notado porque en producción real Hanoy siempre interviene antes de que el bot llegue a un 2do turno propio. Diagnosticado con un canal temporal de logging a `llm_call_logs.error` (los logs de la función no eran legibles desde este entorno): Gemini 3.5 Flash-Lite copiaba literal una frase de ejemplo entre comillas del prompt como si fuera la respuesta final, sin envolverla en JSON — efecto "priming". Fix: sacar frases citadas literales que coinciden con el `reply` esperado, pedir "con tus propias palabras" en su lugar. Diagnóstico temporal removido antes de commitear. **Confirmado por Diego en vivo: "ya el flujo avanza correctamente."** `deno check` sigue sin poder correrse en este entorno (deno no instalado) — pendiente cuando haya acceso. Commiteado y pusheado a `feat/pwa-minimo`.)
> Update previo: 7 Ago 2026 (**Fase 0 completa (solo diseño, cero código tocado) — "Motor híbrido FAQ-first + LLM-fallback"**: fusiona lo que el manual humano llama Fase 3/Fase 4 en un solo agente con capa de FAQ determinista antes del LLM, y de paso rescata un dead-end real del bot clásico. Ver checkpoint ▼ "7 Ago 2026" para el detalle completo. Resumen: partió de un pedido de `/modo-estrategia` (playbook de instalación + entrenamiento del agente, publicado como Artifact) que llevó a `/modo-dev` a diseñar la fusión técnica. Hallazgo de código que cambió el plan: hoy el SDR gasta 1 llamada LLM en el 100% de los turnos — las FAQ solo son contexto del prompt, nunca se intentan solas primero. Validado contra datos reales (3,040 mensajes, las 7 orgs, clásico + SDR): intención mixta en un mensaje es casi inexistente (1/3040, no se diseña para eso); SÍ es real que preguntas de FAQ a mitad de un flujo rígido caen en "opción no válida" en el bot CLÁSICO (Medilaser, Wilmer, clientes pagando) — se agregó como fast-follow en el MISMO plan (mismo matcher, mismo filtro de dominio). Estimado: 3-5h código + 45-90 min QA. **Pendiente: Fase 1 (plan mode) en la próxima sesión de `/modo-dev`** — nada de código tocado hoy. Playbook `playbook-analisis-bot.md` ganó una Fase 3 nueva (casos límite/dead-ends, local o global) como subproducto de la validación. Manual visual: https://claude.ai/code/artifact/9013680c-1bfc-404e-b447-0b686420e66e)
> Update previo: 6 Ago 2026 PM v2 (**Inbox: el buscador no encontraba conversaciones fuera de las 50 más recientes → fix de búsqueda server-side contra toda la org**. Diego no podía encontrar en el inbox de Hanoy conversaciones que sabía que existían. Causa: `useConversations.ts` trae solo las 50 conversaciones más recientes por actividad (`fetchConversations`), y el buscador filtraba client-side sobre esa lista ya cargada — Hanoy tiene 188 conversaciones totales, cualquier paciente fuera del top-50 era invisible para el buscador sin ningún aviso. Fix: nuevo `searchConversationsInOrg()` + hook `useConversationsSearch()` (`useConversations.ts`, mismo patrón de debounce 300ms que `usePatientsSearch.ts` ya usa para pacientes) — con 2+ caracteres dispara una query server-side contra TODA la organización (ILIKE sobre nombre/teléfono, mismo criterio ya probado en `searchPatients()`, sin `unaccent` — Postgres no lo tiene habilitado, trade-off aceptado por Diego). `InboxList.tsx` usa los resultados de búsqueda cuando hay 2+ caracteres, reusando `filterConversations()` (con `searchQuery=""`) para aplicar tabs/línea sin duplicar esa lógica; la vista sin buscar (top 50) queda intacta. `tsc --noEmit` limpio. Verificado contra datos reales: la conversación de Hanoy del 25 Jul que Diego no encontraba (fuera del top-50) aparece con la query nueva. **Sin deployar a Vercel** — solo probado en servidor local (`npm run dev`), pendiente que Diego confirme visualmente y decida cuándo pushear TODO lo de hoy (media+SDR+esto).)
> Update previo: 6 Ago 2026 PM (**SDR: 2 bugs reales de producción encontrados por Diego revisando el inbox de Hanoy en vivo → 3 fixes de código + 1 fix de dato**. Ver checkpoint ▼ "6 Ago 2026 PM" para el detalle completo. Origen: Diego vio timeline real de un paciente (Alejandro) donde el SDR devolvió una FAQ totalmente ajena a la pregunta ("cuál es el precio de la evaluación?" → FAQ de "atienden con cita o por orden de llegada?"), repitió el mismo error al insistir, y luego el bot mandó un mensaje superpuesto justo cuando Hanoy contestó manual desde su celular. Diagnóstico con Explore agent + verificación SQL contra datos reales: (1) **el LLM del SDR falló esa vez y el router degrada "con gracia" al buscador de FAQ clásico (pre-SDR) — pero ese estado (`faq_search`) no es reconocido por el router del SDR, dejando la sesión BLOQUEADA PERMANENTEMENTE fuera del SDR** — FIX: si el fallback clásico aterriza en `faq_search`, se preserva el estado SDR original para que el siguiente mensaje reintente el LLM (`bot-handler/index.ts`, case `sdr_chat`/`sdr_booking`); (2) el buscador de FAQ clásico (`searchFAQ`) sumaba puntaje por CUALQUIER palabra de 3+ letras compartida con el texto de la pregunta de la FAQ, sin filtrar conectores genéricos — FIX: ese fallback ahora solo corre en FAQs SIN keywords configuradas (aplica a TODAS las orgs, no solo SDR); (3) **verificación en vivo reveló que el fix #2 no alcanzaba para el caso reportado** — la FAQ ofensora SÍ tenía keywords, pero una de ellas ("cita") era tan genérica que sola alcanzaba el umbral; se revisó que bloquear FAQ para preguntas de precio en el flujo clásico habría roto 3 orgs reales con FAQs de precio legítimas (Wilmer ×3, Ecoclinicas, Medilaser) — decisión: NO es bug de código, es dato mal configurado — **FIX de dato**: se quitó "cita" de las keywords de esa FAQ puntual de Hanoy vía SQL, verificado que ya no matchea; (4) el chequeo de "¿ya contestó un humano?" (Coexistence) solo corría UNA vez al llegar el mensaje del paciente, nunca se re-evaluaba antes de que el bot efectivamente enviara — con el debounce de 10-30s (agrupar mensajes fragmentados) hay tiempo de sobra para que un humano conteste desde el celular físico sin que el bot se entere — FIX: `routeToBotHandler` (`_shared/bot-routing.ts`, punto único usado por meta-webhook Y bot-debounce-processor) re-consulta `conversations.status` justo antes de enviar y aborta silencioso si ya es `human_active`. `deno check` limpio en los 3 archivos tocados; tests (`sdr-guards`+`honduras-intents`) en el mismo baseline de siempre (42 passed/4 failed preexistentes, confirmado sin regresión). Deployado: `bot-handler`, `meta-webhook`, `bot-debounce-processor` (los 2 últimos por compartir `bot-routing.ts`). **Pendiente:** confirmación de Diego en campo la próxima vez que se dé un caso similar.)
> Update previo: 6 Ago 2026 (**Media entrante rota en el inbox (stickers invisibles + imágenes/video pegados en "Procesando...") → 2 bugs de código + mislabel de video corregidos + backfill de 10 mensajes históricos**. Ver checkpoint ▼ "6 Ago 2026" para el detalle completo. Resumen: (1) `extractMediaFromMetaMessage()` nunca tuvo `case "sticker"` — caían como `text`/`body:null`, burbuja vacía en el inbox, **FIX aplicado y deployado** (nuevo `message_type` real `sticker`, migración de CHECK constraint); (2) `dispatchProcessMediaAsync` en `meta-webhook` disparaba el fetch hacia `process-media-async` sin `EdgeRuntime.waitUntil()` — mismo patrón de condición de carrera ya diagnosticado el 27 Jul para el debounce del bot, **FIX aplicado**; (3) video dejó de remaparse a `"document"` (decisión de scope de Sprint 1) — ahora `message_type="video"` real, con reproductor propio en el inbox; (4) bucket `conversation-media` no aceptaba `video/mp4` — bug encontrado EN VIVO al correr el backfill (un video real de Orthos del mismo día falló ahí), corregido con una migración chica. **Backfill de los 10 mensajes históricos afectados** (función temporal de una sola corrida, reusando `_shared/meta-media.ts` directo — sin depender del `INTERNAL_FUNCTION_SECRET` no legible): 1 arreglado del todo (video Orthos), 3 con tipo corregido pero media irrecuperable (Meta ya no la deja descargar — más vieja del 28 May) marcados con sentinel `unavailable:` en vez de spinner infinito, 6 sin arreglar (stickers de una sesión de pruebas del 11 May en el org demo de OrionCare, `conversation_id` NULL por un bug de conversación distinto y más viejo — sin conversación asociada no hay dónde mostrarlos, data de prueba no de cliente real, no se fuerza). `tsc --noEmit` limpio, `deno check` en baseline preexistente (51 errores, mismos con y sin el diff — confirmado con `git stash`). Migraciones aplicadas directo vía `supabase db query --file` (no `db push` — hay drift real entre historial de migraciones local/remoto, documentado abajo). **Pendiente:** QA visual de Diego en el inbox real (Wilmer, Orthos) — no se pudo probar en navegador esta sesión. Ver Sesion 6 Ago al final.)
> Update previo: 27 Jul 2026 PM (**"El bot no respondió" en la línea real de Hanoy → diagnóstico + 2 fixes de código + fix de copy del saludo**. Ver checkpoint ▼ "27 Jul 2026 PM" para el detalle completo. Resumen: (1) diagnosticado que un mensaje real de Hanoy nunca llegó a `bot-handler` — causa: el despacho fire-and-forget de `meta-webhook` hacia `bot-debounce-processor` (mecanismo de "agrupar mensajes fragmentados" de esa misma mañana) no usaba `EdgeRuntime.waitUntil()`, condición de carrera real donde el runtime puede matar el fetch en vuelo apenas el handler responde a Meta — **FIX aplicado y deployado**; (2) hallazgo en paralelo: el cron de respaldo `bot-debounce-sweep-stuck` (jobid 13) lleva fallando 401 desde que se creó esa misma mañana porque manda `apikey`/`Authorization: Bearer <anon>` en vez del `x-internal-secret` que la función exige — **FIX PENDIENTE**, bloqueado por no tener el valor de `INTERNAL_FUNCTION_SECRET` (no se puede leer vía SQL/MCP); (3) Diego encontró en paralelo que la cuenta de WhatsApp Business de Hanoy no tenía método de pago en Meta — se lo pidió agregar; causa raíz del silencio del 27 Jul no aislada 100% entre el bug de código y esto (pueden ser ambas) — Hanoy tiene el hábito de dejar la tarjeta vacía entre campañas de ads, riesgo recurrente documentado en memoria persistente (`project_hanoy-billing-riesgo-bot-silencio.md`); (4) de paso, ajustado el saludo del bot SDR: ya no pregunta "¿en qué tratamiento está interesado?" al primer contacto — ahora usa una identidad de clínica calculada server-side ("del Dr. X"/"de la Dra. X", derivada del `prefix` de `doctors`/nombre de `clinics`, con fallback a la marca tal cual si no arranca con Dr/Dra) en vez de dejar que el LLM la improvise. Diego probó 2 veces reseteando la sesión del Demo Bot pero no confirmó explícitamente el resultado antes de cerrar la sesión.)
> Update previo: 27 Jul 2026 (**Bot SDR: QA en vivo con mensajes reales contra el Demo Bot (org OrionCare, catálogo/FAQs de Hanoy duplicados para probar) → 6 bugs reales cazados y corregidos + PILOTO ACTIVADO en la clínica real de Hanoy**. Ver checkpoint ▼ "27 Jul 2026" para el detalle completo. Resumen: (1) media sin texto ya no manda el mensaje canned, silencio real; (2) confirma directo cuando pide día+hora exacta libre en vez de menú redundante; (3) `parseDateHint` reconoce "fin de semana"; (4) preguntas informativas sobre una cita existente ("cuándo es mi cita?") ya no disparan el menú de reagendar/cancelar del detector clásico; (5) rechazar una cita ofrecida y pedir otro día en `booking_confirm` ya no cae a "Opción no válida" — re-ofrece slots reales; (6) guard de precios acotado al servicio en foco (no todo el catálogo) + "si" condicional ya no se confunde con confirmación. **Activado en producción para Dra Hanoy Medina** (`bot_enabled=true`, `sdr_mode_enabled=true`) tras decisión explícita de Diego — arranca la semana de supervisión del checklist de piloto. Commit `6f99c29` en `feat/pwa-minimo`, pusheado. Pendiente sin resolver, anotado y aceptado por Diego: `Prótesis Total` (servicio real de Hanoy) queda con `requires_prior_consult=false` y sin precio — el bot dirá "se determina en evaluación" aunque el flag diga que no la necesita; `Limpieza Dental` sí se corrigió (Diego le cargó precio real L1100 en vivo). 4 tests preexistentes de `honduras-intents.test.ts` siguen fallando (confirmado con `git stash` que ya fallaban antes de esta sesión) — no se tocaron, quedan como deuda a revisar aparte.)
> Update previo: 24 Jul 2026 PM v2 (**BOT SDR HÍBRIDO: Fases 0-2b + hardening de fuente-de-verdad, TODO EN UN DÍA** — ver checkpoint ▼ "24 Jul PM" abajo. Resumen: fix 12h en 4 vistas; medición pendiente de humanización corrida (meta >50% SUPERADA: 68% agregado, Wilmer 0%→80%); bot SDR Fases 0-2b completas (schema+RLS, capa LLM, benchmark → **Gemini 3.5 Flash-Lite ganador**, integración a bot-handler + agendamiento conversacional); QA real de Diego cazó 3 bugs de UX (corregidos) + 1 bug de datos (FAQ de horario desactualizada — generalizado a un principio nuevo: horario/precio/especialidad SIEMPRE desde la plataforma, nunca desde FAQ, con auditoría completa de todas las orgs) + 1 bug de código (disponibilidad sin tratamiento identificado caía en silencio) + 1 hallazgo de arquitectura (el horario real vive en `calendar_schedules`, no en `doctor_schedules` — se corrigió antes de asentarse). **LECCIÓN OPERATIVA GRAVE:** `bot_sessions` tiene `ON DELETE CASCADE` hacia `bot_conversation_logs` — un `DELETE FROM bot_sessions` para "destrabar" una sesión borra el historial real de esa conversación. Pasó una vez con datos de prueba de Diego. **Regla nueva: para resetear una sesión atascada, usar `UPDATE bot_sessions SET state=...`, NUNCA `DELETE`, salvo que el historial sea sacrificable (sesiones de prueba propias).** Además: eliminado `price_is_public` (derivado de `requires_prior_consult`), handoff SILENCIOSO hacia el paciente en todo el SDR (con registro en `bot_faq_proposals`), y fix de Coexistence (`conversations.bot_silenced_until` — el bot ya no habla encima de un humano que contesta desde el celular físico, afecta a TODOS los clientes). **SESIÓN CERRADA 24 Jul PM — 11 commits en `feat/pwa-minimo`, pusheados.** SDR sigue prendido en Demo Bot — Diego puede seguir probando cuando quiera.)
> Update previo: 24 Jul 2026 (**Vista mensual renombrada a "Calendario" + FAB "Nueva cita" embebido en Drawer + pulido**. Página `AgendaSemanal.tsx` renombrada completa a `Calendario.tsx` (archivo, ruta `/agenda-semanal`→`/calendario`, ~20 referencias: labels de sidebar ×4, título de header, redirects de login/onboarding/super-admin) — ya no era semanal. `MonthGrid.tsx` ganó `headerExtra` (fusiona el botón "Hoy" en la misma fila del header, ahorra espacio), `swipeable` (gestos touch nativos sin librería para navegar mes anterior/siguiente, con guard de `isLoading`), `largeDays` (cuadros el doble de altos, pedido explícito de Diego) y marca el día de hoy con borde/texto primary siempre (sin prop). FAB "+" abre un Drawer (`vaul`) con TODO el flujo de "Nueva cita" embebido — para esto se extrajo la lógica/UI completa de `NuevaCita.tsx` (742 líneas) a `src/components/NuevaCitaComposer.tsx` reusable, dejando `NuevaCita.tsx` como wrapper delgado de la página `/citas/nueva`; el drawer precarga la fecha (día tocado si es del mes visible, si no `undefined` → el composer default-selecciona hoy) y al agendar con éxito cierra solo + refetch de la agenda. Tres bugs reales de `vaul` investigados leyendo su código fuente (`node_modules/vaul/dist/index.mjs`) y corregidos con props documentadas in-code: `repositionInputs={false}` (vaul redimensiona el drawer en vivo con el teclado abierto, pensado para bottom-sheets cortos no un form de 100dvh), `dismissible={false}` (vaul cierra por distancia O por velocidad — `VELOCITY_THRESHOLD=0.4` interno no configurable — un swipe rápido cerraba con mínimo arrastre pese a `closeThreshold`), `nested` en el drawer interno de fecha/hora (bug de scroll-lock de body en iOS Safari con 2 `Drawer.Root` anidados). Bug de catálogo de servicios: `useAppointmentComposer` mostraba el buscador de profesional por medio segundo al abrir (el `path` se derivaba mal a `'duration'` mientras `catalog` seguía en `[]` por el fetch async) — fix con flag `isLoadingCatalog` nuevo. Cierre de sesión: pase de limpieza con 4 agentes en paralelo (reuso/simplificación/eficiencia/altitud) sobre todo el diff — aplicado: `useWeeklyAgenda.ts` borrado (quedó sin uso), ternario de 4 niveles en `MonthGrid` aplanado a función, guard `isLoading` simétrico en botones prev/next, filtro siempre-true en `useMonthlyAgenda` simplificado, condición de carrera real en `useMonthlyAgenda.fetchAgenda` corregida (mismo patrón `cancelled`/request-id que `useAppointmentComposer`), guard de montaje redundante en el Drawer removido (causaba glitch visual, verificado en vivo que el reset de estado sigue funcionando vía Presence de Radix). Pendiente identificado pero NO aplicado (fuera del diff de la sesión): `NewScheduleExceptionModal.tsx` (bloqueador de horario) tiene el mismo Drawer full-screen con un input de texto y nunca recibió `repositionInputs`/`dismissible` — mismo bug esperable, candidato a próxima sesión corta. También pendiente: formateo de hora 12h duplicado 4 veces en el repo y bloque de timeline de citas duplicado ~80 líneas entre `DayAppointmentsModal.tsx`/`AgendaMedico.tsx` — deuda preexistente, no de esta sesión. Verificado en vivo con Chrome real (truco `isMobile=true` temporal, revertido) en cada paso — incl. drag-to-close con velocidad alta confirmando que `dismissible={false}` lo bloquea, y reset de estado del composer tras quitar el guard de montaje. `tsc` + `vite build` limpios en cada commit de esta sesión.)
> Update previo: 23 Jul 2026 (**Vista mensual de agenda — ajuste post-entrega: solo Mes + modal de día** — Diego probó la primera versión (toggle Semana/Mes) y pidió 2 cambios de layout, sin Fase 0 porque no tocan datos ni reglas: (1) quitar el toggle, la pantalla ahora es SOLO el calendario mensual (se eliminó todo el código de modo semana: `useWeeklyAgenda`, tira de 7 días, nav de semana — `useWeeklyAgenda.ts` se dejó intacto por si se reusa en otro lado, solo se sacó su uso de esta pantalla); (2) el timeline de citas + el filtro de médico (antes inline debajo del calendario) ahora viven en un modal nuevo (`src/components/DayAppointmentsModal.tsx`, patrón Drawer mobile / Dialog desktop calcado de `NewScheduleExceptionModal.tsx`, con `useIsMobile()`) que se abre al tocar un día — libera espacio para que el grid mensual se vea más grande. Bug propio detectado y corregido en el camino: al armar la primera versión de esta pantalla se agregó una barra de navegación de mes duplicada (prev/Hoy/next) que competía con la que `MonthGrid` YA trae en su propio header — se dejó solo un botón "Hoy" suelto y se removió la nav duplicada. Verificado en vivo: modal abre con fecha+conteo correctos, timeline con línea/punto intacto, filtro de médico funcional, "Reagendar" anida correctamente `RescheduleModal` (Dialog dentro de Dialog/Drawer vía portals de Radix, sin conflicto), día vacío muestra el Alert correcto, Drawer mobile probado con el truco de forzar `isMobile=true` (revertido y confirmado con `grep`). `tsc` + `vite build` limpios.
> Update previo: 23 Jul 2026 (**Vista mensual de agenda ENTREGADA** — Fase 0 en conversación con Diego (Hanoy/UNIMED pidieron algo tipo TimeTree) + mockup de Stitch aprobado, luego implementada: toggle "Semana"/"Mes" en `AgendaSemanal.tsx` (mismo patrón `ToggleGroup` del bloqueador de horario); `MonthGrid.tsx` con 2 props nuevos retrocompatibles (`markedDays: Set<string>` → punto indicador bajo el día con citas reales; `disablePast=false` → días pasados seleccionables, a diferencia del booking); hook nuevo `useMonthlyAgenda.ts` (hermano de `useWeeklyAgenda.ts`, mismo RPC `get_weekly_agenda` — genérico en rango de fechas, sin cambios de BD — pero pidiendo el mes completo vía `startOfMonth`/`endOfMonth`, sin el prefetch agresivo de semanas adyacentes que sí tiene el hook semanal). El calendario mensual NUNCA colapsa (sin prop `collapsible`, a diferencia del uso en el bloqueador de horario) — decisión explícita de Diego. Timeline de citas rediseñado con línea vertical + punto conectando cada hora (antes solo tiempo+card sin conector visual) y reusado en AMBOS modos (semana y mes) — mismo `StatusBadge` y botón "Reagendar"/`RescheduleModal` de siempre. Verificado en vivo contra datos reales (SQL directo a `appointments` confirmando que los puntos del calendario coinciden exacto con los días con citas, incl. que los domingos sin citas NO muestran punto); probado con rol Médico y Administrador (selector de doctor sigue funcionando); `tsc` + `vite build` limpios. Pendiente: QA de viewport mobile real (`resize_window` sigue sin funcionar en este entorno — ver `reference_browser-resize-window-no-funciona.md`; esta pantalla no usa `useIsMobile()`, es 100% CSS responsive `md:`, así que el truco de forzar el hook no aplica aquí — Diego debe confirmar en su celular).
> Update previo: 23 Jul 2026 (**Fix de UX v7: hueco vertical en modo día completo (mobile)** — Diego reportó, tras el v6, que en modo "día completo" quedaba demasiado espacio entre el calendario y la fila "Bloqueo de N días / Limpiar", solo en mobile. Causa: la raíz de `StepPicker` tenía `flex-1` pensado para el modo "por horas" (repartir espacio entre calendario+horarios); en día completo no hay columna de horarios con quien compartir ese espacio, así que el `flex-1` estiraba el calendario para llenar el alto disponible del Drawer (`h-[100dvh]`, alto fijo real) dejando el hueco antes de la fila resumen. En desktop no se notaba porque el padre (`DialogContent`) usa `display:grid`, donde `flex-1` no tiene efecto (no hay "espacio sobrante" que reciba flex-grow). Fix: `flex-1` ahora condicional a `!fullDayMode` (`cn('flex flex-col lg:flex-row gap-6', !fullDayMode && 'flex-1')`) — modo por horas intacto. Verificado forzando `isMobile=true` temporalmente (mismo método que v6, revertido): hueco desaparecido en día completo, por horas sin regresión. Ver checkpoint ▼ "23 Jul 2026 — Fix hueco vertical v7".)
...
## ▼ CHECKPOINT 10 Ago 2026 — Piloto bienes raíces (Bruno): Propiedades + integración bot-handler completos

**Contexto:** Bruno (novio de Hanoy Medina, agente inmobiliario independiente) fue confirmado en piloto el 8 Ago (ver `project_piloto-bienes-raices-bruno.md` para el detalle de negocio/pricing/investigación técnica de canales de Meta). Fechas comprometidas: semana del 10/8 = entidad+interfaz, semana del 17/8 = vinculación del número + arranque real. Esta sesión cubrió ambas fases completas.

### Parte 1 — Catálogo de Propiedades (Fases 1-3)

Tabla `properties` (org-scoped, código autogenerado `COD-NNN`, RLS mirror de `service_types`) + `appointments.property_id` (FK opcional) + `organizations.vertical` (`CHECK 'clinica'|'bienes_raices'`, default `'clinica'`) + bucket `property-photos`. Capa de datos: `propertiesApi.ts` + `useProperties.ts` (mismo patrón `usePromotions.ts`). UI: patrón **Hub + Drawer** nuevo (nombrado y documentado en `CLAUDE.md` Fase 3) — 1 sola ruta `/propiedades`, crear/ver/editar vive en `PropertyDrawer.tsx` con 2 modos internos, nunca rutas separadas. Nav item "Propiedades" condicional a `vertical==='bienes_raices'`, solo en bloque Admin (Bruno opera solo).

**Bonus de la sesión:** se extrajo `ResponsiveModal` (`src/components/ui/responsive-modal.tsx`) de la duplicación Drawer/Dialog en `DayAppointmentsModal.tsx`/`NewScheduleExceptionModal.tsx` — de paso corrigió que esos 2 componentes NO tenían `dismissible={false}` pese a necesitarlo (bug real de swipe-close).

**QA en vivo con Diego, varias rondas de ajuste** (todas cerradas mismo día): quitar subtítulo redundante, buscador+filtro en una fila, grid fijo 2 columnas, bug real de estado no reflejado visualmente al cambiar status desde el Drawer (corregido), `property_type` de texto libre a `Select` cerrado con `CHECK`, campos nuevos (`construction_size_m2`/`parking_spots`/`front_yard`/`back_yard`), componente nuevo reusable `NumericInput`, iteración de 2 rondas en el posicionamiento del mensaje de confirmación (terminó como pill flotante emerald, `position:absolute` dentro de un wrapper `min-h-full`). **QA aprobado por Diego.**

**Detalle completo:** ver `project_piloto-bienes-raices-bruno.md`, secciones Fase 1/2/3 + rondas de ajuste.

### Parte 2 — Integración bot-handler (Fases 0-3)

**Diseño (Fase 0, en conversación con Diego):** decisión de arquitectura explícita — el flujo nuevo DUPLICA de `sdr.ts` lo que haga falta en vez de reusarlo por herencia (sin sistema universal por rubro todavía; ese se construye después de 2+ casos reales, mismo criterio que `ResponsiveModal`). Un prompt propio por rubro (`realestate-prompt.ts`), no uno "global" compartido con medicina — persona/tono se copian y adaptan, conocimiento es rubro-específico.

**Mecanismo de matching (decidido 8 Ago, implementado hoy):** código corto por propiedad (`COD-104`), matcheado en `referral.headline`/`body` de Meta (canal ads/posts) O en texto plano (canal Marketplace/link manual `wa.me?text=COD-104`) — un solo lookup para los 2 canales. Resuelto UNA vez en `meta-webhook`, ANTES de encolar al debounce, para que sobreviva mensajes fragmentados (el `referral` normalmente solo viene en el primer mensaje).

**Archivos nuevos:**
- `_shared/properties.ts` — `matchPropertyCode` (regex `COD-\d+`, sin fuzzy), `getPropertyByCode`, `getPropertyById`.
- `_shared/realestate-prompt.ts` — mismo shape que `sdr-prompt.ts`, ficha de UNA propiedad en vez de catálogo, regla "ya no disponible" cuando `status` no es disponible/reservada.
- `bot-handler/realestate.ts` (~442 líneas) — `maybeHandleRealEstate` (entry point, mismo contrato que `maybeHandleSdr`: `null` cae al flujo clásico en cualquier incertidumbre). Implementaciones PROPIAS de `offerRealEstateSlots`/`acceptRealEstateSlot` sobre primitivas de bajo nivel confirmadas agnósticas (`fetchSlotsForDate`, `pickSlots`, `matchOfferedSlot`) — se evitó reusar `offerSlotsForDate`/`acceptSlot` de `sdr.ts` porque toman `ctx: SdrContext` médico-específico sin forma segura de verificar un stub sin `deno check`.

**Cambios quirúrgicos:**
- `sdr.ts` ganó `export` en varias funciones (cero cambio de comportamiento, solo visibilidad) — se revirtió el export de `offerSlotsForDate`/`acceptSlot` específicamente tras decidir no reusarlas.
- `meta-webhook/index.ts` — tipo `MetaMessage.referral`, `handleIncomingMessage` gana `vertical` param + bloque de matching post-gate-histórico, query de línea suma `organizations(vertical)`.
- `bot-handler/index.ts` — import de `realestate.ts`, estados `'realestate_chat'`/`'realestate_booking'`, `session.context.orgVertical` cacheado en la consulta a `whatsapp_lines` ya existente, branch único `orgVertical==='bienes_raices' ? maybeHandleRealEstate : maybeHandleSdr`, 1 línea nueva en `createAppointmentWithPatient` (`property_id`).

**Bug real encontrado y corregido ANTES de deployar:** `loadSoleAgentDoctor` (primer draft) consultaba `.from("calendars").eq("doctor_id", ...)` — `execute_sql` contra `information_schema.columns` probó que `calendars` NO tiene columna `doctor_id`. La relación real es vía `calendar_doctors`/`whatsapp_line_doctors` (junction tables). Corregido para usar los deps ya inyectados (`lineCalendarForDoctor`/`firstActiveCalendarId`), mismo mecanismo que `sdr.ts` usa para auto-asignar calendario.

**Hallazgo que redujo trabajo:** `acceptSlot` en `sdr.ts` no crea la cita — transiciona a `booking_confirm`, que el handler CLÁSICO (compartido) procesa vía `createAppointmentWithPatient`. Agregar soporte de propiedad fue 1 sola línea, no duplicar la creación de citas. También: `useAppointmentComposer` (frontend) ya tiene un path `'duration'` para orgs sin `service_types` — la org de Bruno cae ahí gratis (sin código nuevo) porque propiedades vive en tabla separada, nunca en `service_types`.

**Fix de paso (mismo día, no planeado):** el flujo clásico de reagendar/cancelar (reusado tal cual para visitas) tenía copy médico hardcodeado (`🩺 ${doctorName}`) en 10 líneas, sin mencionar la propiedad. Decisión de Diego: quitar el emoji para TODAS las orgs (cosmético, riesgo cero) en vez de condicionar por rubro — deployado por separado, antes del resto de Fase 3. Que el mensaje mencione la propiedad queda en backlog (`project_backlog-bienes-raices.md`).

**Deploys (todos con AskUserQuestion previo, confirmados por Diego):** `meta-webhook` (Fase 2, matching), `bot-handler` (emoji fix, aislado), `bot-handler` (Fase 3 completa — el más grande, revisado línea por línea del diff completo antes de preguntar). Los 3 con logs post-deploy verificados limpios.

**Convención nueva establecida:** cada rubro no-médico tiene su propio archivo de backlog (`project_backlog-<rubro>.md`), separado del archivo de progreso activo — para no mezclar "en construcción" con "diferido a propósito". Primera instancia: `project_backlog-bienes-raices.md` (matching por `source_id` de Meta, mención de propiedad en reagendar/cancelar).

**Cuidado real de la sesión (no repetir):** al probar el nav condicional se tocó por error `organizations.vertical` de la org demo `c8b1c83b` (OrionCare) — que también es herramienta de ventas activa, no solo org de prueba — revertido de inmediato. Ver `feedback_demo-org-no-tocar-flags-globales.md`.

**Verificación:** `tsc --noEmit` limpio, `npm run build` limpio (incluye `PropertiesPage` en su propio chunk), revisión manual línea por línea de todo el diff Deno (sin `deno check` disponible en este entorno — sigue pendiente correrlo cuando haya acceso). `npm run lint` con deuda preexistente en TODO el repo (`no-explicit-any`, componentes UI de shadcn incluidos) — confirmado no-regresión comparando contra archivos nunca tocados esta sesión.

**Estado final:** commiteado (`7f73465` ResponsiveModal, `38c617a` Propiedades, `88fd3f3` integración bot-handler) y pusheado a `feat/pwa-minimo`. Código deployado y "en oscuro" (sin tráfico real) hasta que exista la org de Bruno.

**Pendiente semana del 17/8:** confirmar nombre real del negocio de Bruno → crear su organización (`vertical='bienes_raices'`) + fila en `doctors` (prerrequisito: `appointments.doctor_id` es `NOT NULL`) + conectar su número de WhatsApp → primera prueba end-to-end real.

---

## ▼ CHECKPOINT 9 Ago 2026 — SDR: pregunta de interés antes de invitar a agendar

**Contexto:** Diego revisó en vivo el inbox de Hanoy y notó que en 2 conversaciones (+50432348069, +50433053172, viernes/sábado 8-9 Ago) el SDR respondió correcto pero cerró en seco pidiendo agendar, y el lead se quedó callado hasta que Hanoy intervino manualmente. Hipótesis de Diego: falta "chit-chat" para que el paciente se sienta acompañado antes de agendar, como hace Hanoy.

**Diagnóstico con datos reales (no hipótesis) — las 7 conversaciones del SDR desde su activación (27 Jul):**
- 3/7 (43%) quedaron MUERTAS tras la respuesta del bot — sin que nadie, ni bot ni humano, reintentara nada.
- En las 4 restantes, Hanoy intervino manualmente (mensajes `source="assistant"`, echo de su WhatsApp físico vía Coexistence — confirmado en código, `silenceForPhysicalReply()` en `_shared/conversations.ts`, apaga el bot hasta medianoche apenas ella contesta desde el celular). Las 4 veces, sin excepción, ella hizo un saludo cálido o una pregunta específica sobre el caso ANTES de invitar a agendar — nunca replicó el patrón seco del bot. Las 2 que cerraron cita real son justo esas.
- Comparación con el bot CLÁSICO (Wilmer, Medilaser, Consultorio Familiar): ~34% de sesiones que tocan `faq_search` mueren justo ahí sin elegir "otra pregunta"/"menú" — misma magnitud que el SDR, pero causa distinta (fricción de tener que escribir un número, no de tono).

**Decisión de Diego:** agregar la pregunta de interés (ver cambio abajo) para ver cómo se desenvuelve. El 43% de leads sin seguimiento y el 34% del clásico quedan backlogueados aparte — explícitamente NO se trabajan ahora.

**Cambio implementado (Fase 1 vía plan mode, plan `.claude/plans/squishy-brewing-spring.md`):**
- `_shared/sdr-prompt.ts`: nuevo campo `interest_question_asked: boolean` en `SdrLLMOutput` + parseo en `parseSdrOutput`; nuevo opt `alreadyAskedInterestQuestion?: boolean` en `SdrPromptOptions`; reglas 3-4 del guión reescritas como regla 3 (pregunta de interés, condicional al flag, cada rama autocontenida) + regla 4 (ex-regla 5, sin opinión clínica).
- `bot-handler/sdr.ts` (`handleSdrChat`): pasa `alreadyAskedInterestQuestion: !!session.context.sdrAskedInterestQuestion` al prompt; tras recibir `out` y pasar el guard de precio, marca `session.context.sdrAskedInterestQuestion = true` si `out.interest_question_asked`. Flag se limpia junto con `sdrHistory`/`sdrInterestServiceId` cuando una sesión revive (`completed`/`expired`) — un paciente que vuelve con un caso nuevo recupera su pregunta de interés.
- Sin migración (mismo patrón que `sdrBookingSetup`: flag ad-hoc en `session.context`, JSONB schemaless). "Si el paciente pide agendar explícito, saltarse la pregunta" queda resuelto gratis por el código existente: la rama `wantsBooking` (`sdr.ts:293-313`) nunca pasa por `out.reply`, va directo a `handleSdrBooking`.

**QA en vivo con Diego (Demo Bot, org OrionCare) — 2 bugs reales cazados y corregidos antes de cerrar:**
1. **Empatía genérica no pedida:** el primer intento del bot abrió con "entiendo lo incómodo que es eso" — frase enlatada que Diego señaló como no-natural (revisado contra las conversaciones reales de Hanoy: CERO veces usa una frase de empatía antes de preguntar, va directo a la pregunta específica: "¿Sería uno de enfrente?", "¿En qué pieza es su endodoncia?"). Fix: el bloque de la pregunta de interés ahora prohíbe explícitamente frases de empatía genérica ("entiendo lo molesto/incómodo que es eso", "lamento escuchar eso": NUNCA) y ancla los ejemplos al tono directo real de Hanoy.
2. **Bug de fondo, MÁS GRAVE, PRE-EXISTENTE (no lo causó esta sesión):** el SDR caía sistemáticamente al bot clásico en el 2do turno de CUALQUIER conversación (confirmado 3/3 veces reproducido en vivo) — mismo patrón exacto ya visto el 5 Ago con Alejandro/Hanoy (documentado en el checkpoint 6 Ago como "el LLM del SDR falló esa vez"), solo que nadie había aislado la causa porque en producción real Hanoy siempre interviene manualmente ANTES de que el bot llegue a un 2do turno propio — el camino nunca se había ejercitado limpio. Diagnóstico: como `get_logs` (MCP) no expone la salida de `console.warn` de las edge functions en este entorno, se armó un canal de diagnóstico temporal escribiendo el texto crudo del LLM directo a `llm_call_logs.error` (columna existente, fire-and-forget) — reveló que **Gemini 3.5 Flash-Lite copiaba literal la frase de ejemplo entre comillas del prompt ("¿Le gustaría que le agende su cita?") como si fuera la respuesta final, sin envolverla en el JSON** — efecto de "priming": un molde citado casi idéntico a la respuesta esperada, en un bloque corto y directivo, hace que el modelo complete/copie en vez de seguir el formato. Fix real: se sacó la frase literal citada de las dos ramas del bloque de interés — ahora pide "cerrar con tus propias palabras (no copies un molde fijo)" en vez de darle el texto exacto a repetir. El diagnóstico temporal (console.warn + insert a llm_call_logs) se removió del código antes de commitear.
3. **Verificado en vivo por Diego tras el fix:** "ya el flujo avanza correctamente" — 2do turno cierra bien, sin caer al clásico.

**Deployado:** `bot-handler` (incluye `_shared/sdr-prompt.ts` empaquetado), 5 iteraciones de deploy durante la sesión de QA, última con el diagnóstico ya removido. **`deno check` sigue sin poder correrse** en este entorno (deno no instalado, no encontrado en PATH ni ubicaciones típicas de Windows) — el bundling de `supabase functions deploy` no tira errores, pero no reemplaza un check real; pendiente correrlo cuando haya acceso a un entorno con deno.

**Pendiente:** confirmar que el bug del 2do turno (Gemini "priming" con frases citadas en el prompt) no tenga otras instancias en el resto de `_shared/sdr-prompt.ts` — no se auditó el prompt completo por otras frases citadas literal que coincidan con el texto esperado de `reply`, solo se corrigieron las 2 detectadas en esta sesión.

---

## ▼ CHECKPOINT 7 Ago 2026 — Fase 0: Motor híbrido FAQ-first + LLM-fallback (+ fast-follow bot clásico)

**Contexto:** sesión mixta `/modo-estrategia` → `/modo-dev`. En estrategia se armó un manual visual (Artifact) del flujo de instalación + entrenamiento del agente, con el pedido explícito de Diego de dejar de narrar "bot clásico" y "SDR" como dos bots separados — un solo agente, una capa de FAQ + una capa de razonamiento, que nunca inventa. Eso llevó a `/modo-dev` a diseñar la fusión técnica real. **Solo diseño esta sesión — cero líneas de código tocadas.**

### Hallazgo de código que corrige el plan (leído en vivo, no memoria)

- `bot-handler/sdr.ts` (`maybeHandleSdr` → `handleSdrChat`): **hoy el SDR llama al LLM (Gemini/Haiku) en el 100% de los turnos**. Las FAQ de la org (`buildSdrContext` en `_shared/sdr-prompt.ts`) se inyectan como CONTEXTO del prompt — el LLM decide si las usa, pero nunca se intenta responder solo con FAQ antes de gastar la llamada. El "híbrido FAQ+LLM" que se venía discutiendo no existe ni parcialmente.
- El matcher determinista que hace falta YA EXISTE: `searchFAQ()` en `bot-handler/index.ts` (~línea 3841) — ponderado por `1/frecuencia` de cada keyword (hardened 6 Ago), con umbral configurable por FAQ vía `bot_faqs.min_match_score` (default 1.0, migración `20260511120001`). No hace falta inventar un matcher nuevo ni migración de schema para la versión mínima.
- El filtro de dominio estructurado (`filterStructuredDomainFaqs`, `filterOrphanServiceFaqs` en `_shared/sdr-prompt.ts`) ya excluye horario/precio/especialidad de lo que el SDR puede citar como FAQ — reusar el mismo criterio en el fast-path, no reinventarlo.
- `bot_faq_proposals` + `registerFaqProposal()` ya auto-pueblan la cola de huecos de conocimiento desde el 24 Jul — no es parte de este proyecto, sigue pendiente aparte (pantalla de aprobación, Fase 3 del proyecto SDR original).

### Fase 0 — las 6 preguntas

1. **Una oración:** antes de llamar al LLM en el turno de ENTRADA del SDR, intentar un match determinista de alta confianza contra las FAQ de la org (excluyendo horario/precio/especialidad); si hay match, responder gratis e instantáneo; si no, seguir el flujo SDR actual sin cambios. El mismo matcher, fast-follow en el mismo plan, rescata preguntas de FAQ que hoy caen en "opción no válida" a mitad de un flujo rígido del bot CLÁSICO (agendar/reagendar/cancelar).
2. **Actores y permisos:** sin cambios — invisible al paciente, no toca roles. Mismo aislamiento por organización que ya usan `searchFAQ`/`buildSdrContext`.
3. **Entidades:** ninguna nueva. Reusa `bot_faqs` y `bot_faq_proposals` tal cual. Cero migración para la versión mínima.
4. **Ciclo de vida:**
   - SDR: el fast-path corre ANTES de `handleSdrChat`, **solo en el turno de entrada** (`greeting`/`main_menu`, sin contexto de booking activo) — NUNCA dentro de `sdr_booking` ya en negociación ni `booking_confirm` (ver casos límite). Si matchea, la sesión se queda en `sdr_chat` — nunca en `faq_search` (ese estado fue la causa exacta del bug de bloqueo permanente del 6 Ago, no se reintroduce en el SDR).
   - Clásico (fast-follow): cuando un flujo rígido (`booking_select_*`, `reschedule_*`, `cancel_confirm`) recibe una entrada que hoy dispara "opción no válida", intentar el mismo matcher ANTES de mostrar el error. Si matchea con alta confianza: responder la FAQ y re-mostrar el paso pendiente del flujo (sin perder el lugar donde estaba el paciente — evitar crear un caso límite nuevo). Si no matchea: mismo comportamiento de hoy, sin cambios.
5. **Reglas de negocio:** reusar `searchFAQ` (extraído a `_shared/` para que `sdr.ts` E `index.ts` lo importen sin duplicar lógica); excluir siempre los dominios estructurados; umbral de confianza MÁS estricto que el uso actual del botón "4. FAQ" (acá reemplaza al LLM/al flujo entero, no es una sugerencia con salida de "otra pregunta"); nunca inventa — sin match seguro, cae al comportamiento de hoy en ambos casos.
6. **Casos límite — resueltos con datos reales (no hipótesis), corridos contra las 7 orgs / 3,040 mensajes:**
   - Intención mixta en un solo mensaje ("precio Y agendar" junto): **casi inexistente, 1/3040** (Medilaser, marzo). No se diseña para esto en v1 — el peor caso es que el paciente repregunte, no se inventa nada.
   - Fast-path sin memoria conversacional interrumpiendo un booking activo: **confirmado real** en sesiones SDR de QA (24 Jul, preguntas tipo "cuánto dura el procedimiento?" a mitad de `sdr_booking`) → por eso el fast-path del SDR se limita al turno de ENTRADA solamente.
   - Dead-end real en el bot CLÁSICO (justifica el fast-follow): confirmado con `state_before = state_after AND bot_response ILIKE '%no valida%'` cruzado con estados de flujo, en TODAS las orgs — Medilaser ("Una pregunta atiende los domingos?" a mitad de elegir doctor), Wilmer ("Hola ubicación" a mitad de elegir servicio). 2 clientes reales pagando, afectados HOY, sin SDR de por medio.
   - Org sin FAQs (Consultorio Familiar): el fast-path nunca matchea, cae siempre al camino de hoy — sin caso especial que programar.

**Anti-scope-creep (fuera de esta fase):** sin matching semántico/embeddings en v1 (el keyword-based ya fue hardened 6 Ago; evaluar semántico como v2 solo si hay falsos positivos reales en producción); sin pantalla de aprobación de `bot_faq_proposals`; sin fusionar los flags `bot_enabled`/`sdr_mode_enabled` en uno solo; sin tocar el resto del flujo clásico (solo el punto de "opción no válida" en agendar/reagendar/cancelar).

**Estimado de tiempo:** 3-5 horas de código (extraer `searchFAQ` a `_shared/`, 2 puntos de integración — `sdr.ts` y `index.ts` — umbral, exclusión de dominio, texto de cierre estático, "recordar el paso pendiente" en el fast-follow clásico) + 45-90 min de QA en vivo (Demo Bot + revisión con Diego). Cabe en una sola sesión, mismo ritmo histórico del proyecto SDR (Fase 2b completa se hizo en 1 día).

**Subproducto de la sesión:** `playbook-analisis-bot.md` ganó una Fase 3 nueva ("casos límite / dead-ends de fallback", corre local por org o global entre todas) — quedó documentada ahí, no es exclusiva de este proyecto, sirve para cualquier auditoría futura del bot.

**Pendiente para la próxima sesión de `/modo-dev`:** Fase 1 (plan mode) — diseñar la extracción de `searchFAQ` a `_shared/`, el punto de integración exacto en `sdr.ts` (dentro de `maybeHandleSdr`, antes de `handleSdrChat`) y en `index.ts` (dónde interceptar "opción no válida" antes de responderla en los estados de flujo), el valor del umbral de confianza, y el texto de cierre estático para que la respuesta del fast-path no quede seca. Referencia completa del diseño: memoria persistente `motor-hibrido-faq-llm`.

---

## ▼ CHECKPOINT 6 Ago 2026 PM — SDR: FAQ mal emparejada + bloqueo permanente + overlap bot/humano

**Contexto:** mismo día, sesión aparte — Diego revisó en vivo el inbox de Hanoy (motivado por el fix de media de la mañana) y encontró un caso real donde el SDR falló feo. Diagnóstico completo vía Explore agent + SQL contra datos reales de producción, Fase 0 → plan mode → implementación.

**Los 4 fixes están detallados en la línea "Ultima actualizacion" de arriba.** Nota operativa para la próxima sesión: el punto más importante de este checkpoint es el patrón de verificación — el primer fix de `searchFAQ` (gatear el fallback de overlap a FAQs sin keywords) era correcto pero **insuficiente** para el caso real reportado, porque la FAQ ofensora sí tenía keywords, solo que una era demasiado genérica. Se detectó SOLO porque se verificó contra la FAQ real de Hanoy antes de dar el fix por cerrado — recordatorio de no declarar un fix de matching/scoring resuelto sin probarlo contra el dato real que causó el bug, no solo contra la teoría del algoritmo.

**Riesgo estructural cerrado en la misma sesión:** Diego preguntó si esto podía escalar a futuros clientes sin que nadie audite keywords manualmente. Fase 0 + plan mode + implementación: `searchFAQ` ahora pondera cada keyword por `1/frecuencia` (cuántas FAQs activas del mismo scope org/clínica/doctor comparten esa keyword) en vez de un peso fijo — una keyword única sigue valiendo lo mismo que antes, una compartida por N FAQs vale 1/N. Se autorregula por cliente sin curación manual. Verificado contra datos reales de Wilmer y Hanoy: **hoy ninguna keyword está compartida entre 2+ FAQs de la misma org** (frecuencia 1 en absolutamente todas) — el cambio es matemáticamente un no-op sobre el comportamiento actual (cero riesgo de regresión), la protección se activa sola el día que dos FAQs de un mismo cliente compartan una keyword. `deno check` limpio, mismo baseline de tests (42/4). Deployado.

---

## ▼ CHECKPOINT 6 Ago 2026 — Media entrante rota en el inbox (stickers, imágenes atascadas, video) + backfill

**Contexto:** Diego reportó que stickers/imágenes de pacientes no se veían en el inbox y el bot clásico solo decía que no podía ver el contenido (esa frase ya no la dice desde el 27 Jul, queda en silencio — el problema real era que el inbox tampoco mostraba nada). Sesión completa en `/modo-dev`, Fase 0 → Fase 1 (plan mode) → implementación, plan guardado en `.claude/plans/wondrous-roaming-scott.md`.

**1. Diagnóstico — 2 bugs de código + 1 mislabel deliberado:**
- Sticker: `extractMediaFromMetaMessage()` (`_shared/inbox-messages.ts`) nunca tuvo `case "sticker"` — caía al `default`, se persistía como `message_type: "text"`, `body: null`. Burbuja vacía en el inbox, sin rastro de que el paciente mandó algo.
- Imágenes/documentos pegados en "Procesando...": `dispatchProcessMediaAsync` en `meta-webhook/index.ts` disparaba el fetch hacia `process-media-async` sin `EdgeRuntime.waitUntil()` — mismo patrón de condición de carrera ya diagnosticado el 27 Jul para el debounce del bot (el runtime puede matar el fetch en pleno vuelo apenas el handler responde 200 a Meta).
- Video se remapeaba a `"document"` (decisión deliberada de Sprint 1, "no usa video") — Diego pidió resolverlo de una vez.

**2. Fix de código (deployado):**
- `_shared/inbox-messages.ts`: `MessageType` agrega `"sticker"`/`"video"`; `extractMediaFromMetaMessage` nuevo `case "sticker"`, `case "video"` ya no remapea a document.
- `meta-webhook/index.ts`: `MetaMessage` interface agrega campo `sticker`; `dispatchProcessMediaAsync` envuelto en `EdgeRuntime.waitUntil()` (mismo patrón que `enqueueDebounceMessage`).
- `process-media-async/index.ts`: tipo `ProcessMediaBody.messageType` amplía union — sin cambio de lógica (el branch de Whisper ya está acotado a `=== "audio"`).
- Frontend: `useConversationMessages.ts`/`useRealtimeInbox.ts` amplían el tipo `message_type`; `MessageBubble.tsx` — sticker reusa `ImageBubbleContent` (es WebP, el bucket ya lo aceptaba), video nuevo `VideoBubbleContent` (reproductor `<video controls>`); las 3 (imagen/documento/video) ganan un estado "Contenido ya no disponible" cuando `media_url` trae el sentinel `unavailable:` (ver punto 4); `ConversationListItem.tsx` — preview del último mensaje agrega íconos/label para sticker y video.
- Migración `20260806120000`: `message_logs_message_type_check` amplía a incluir `video`/`sticker`.

**3. Backfill de 10 mensajes históricos afectados (todas las orgs):** función temporal `backfill-media-20260806` (deployada CON verificación JWT normal, invocada una vez vía HTTP con el anon key, después borrada de Supabase y del repo) — reusa `_shared/meta-media.ts` directo (resolve/download/upload) en vez de llamar `process-media-async` por HTTP, para no depender de `INTERNAL_FUNCTION_SECRET` (valor no legible, bloqueador ya conocido de otra tarea). Resultado tras 2 corridas:
- **1 arreglado del todo:** video de Orthos (3 Ago) — ahora se ve real en el inbox.
- **3 con tipo corregido, media irrecuperable** (Meta ya no la deja descargar — la más vieja es del 28 May): video de OrionCare demo, sticker y foto de Wilmer (7 y 23 Jul). Quedan marcados `media_url = 'unavailable:meta-media:{id}'`, el inbox muestra "Contenido ya no disponible" en vez de spinner eterno.
- **6 sin arreglar:** stickers del 11 May en el org demo de OrionCare, mismo número, 11 minutos seguidos — `conversation_id` es NULL (bug de conversación distinto y más viejo, `getOrCreateConversation` falló ese día y cayó al fallback legacy sin conversación). Sin conversación asociada no hay dónde mostrarlos en el inbox aunque se recuperara el archivo — es data de prueba de mayo, no de un cliente real, no se forzó una solución fuera del alcance de hoy.

**4. Hallazgo en vivo durante el backfill (no estaba en el plan original):** el bucket `conversation-media` no tenía `video/mp4`/`video/3gpp` en su `allowed_mime_types` — el video de Orthos falló en el paso de subida a Storage con eso como causa exacta. Migración `20260806130000` lo corrige. De paso, `_shared/meta-media.ts` (`MIME_TO_EXT`) tampoco mapeaba mime de video → los archivos de video quedaban guardados como `.bin` (no rompe reproducción, Storage sirve el `Content-Type` real independiente de la extensión, pero se corrigió para que queden bien nombrados a futuro).

**5. Deploys:** `meta-webhook` y `process-media-async` con `--no-verify-jwt` (patrón del repo). `tsc --noEmit` limpio. `deno check` en baseline preexistente (51 errores en `process-media-async`, mismos con y sin el diff — confirmado con `git stash`, mismo patrón de deuda técnica que `meta-webhook`, ver nota ampliada arriba).

**Pendiente para la próxima sesión:**
1. QA visual de Diego en el inbox real: conversaciones de Wilmer (mensajes del 7 y 23 Jul, deberían mostrar "Contenido ya no disponible") y de Orthos (video del 3 Ago, debería reproducirse).
2. Mandar un sticker y un video reales de prueba a una línea con bot para confirmar el flujo hacia adelante (no se pudo probar en vivo esta sesión, sin acceso a WhatsApp real desde aquí).
3. Migraciones `20260806120000` y `20260806130000` quedan en el repo pero se aplicaron directo (`db query --file`), no vía `db push` — cuando se reconcilie el drift de migraciones (ver Deuda técnica arriba), confirmar que sigan registradas correctamente.

**Addendum (misma sesión) — tarjeta de anuncio de origen (Click-to-WhatsApp Ads):** mientras Diego revisaba el inbox de Hanoy notó que los leads de Facebook/Instagram ads ("¡Hola! Quiero más información") se veían como texto plano, mientras que WhatsApp nativo muestra una tarjeta con la miniatura del anuncio. Confirmado por SQL: Meta manda un objeto `referral` (headline, body del ad, thumbnail_url, video_url, source_url, ctwa_clid) en el primer mensaje de cada conversación que arranca desde un ad — ya vivía en `message_logs.raw_payload` (mismo campo que hizo posible el backfill de arriba) pero nunca se leía ni se mostraba. Fix: `useConversationMessages.ts` ahora selecciona `raw_payload` + expone `getAdReferral()`; `MessageBubble.tsx` nuevo `AdReferralCard` (thumbnail + headline + link al anuncio) se renderiza arriba de la burbuja cuando el mensaje del paciente trae `referral`. Sin cambios de schema (columna ya existía), sin Fase 0 formal (visual/informativo puro, no cambia reglas de negocio — mismo criterio que `feedback_iteracion-fase0-vs-plan-mobile-ui`). `tsc --noEmit` limpio. **Pendiente:** confirmar visualmente en el inbox real de Hanoy que la tarjeta aparece bien.

---

## ▼ CHECKPOINT 27 Jul 2026 PM — "Bot no responde" en línea real de Hanoy → diagnóstico + fixes + copy del saludo

**Contexto:** horas después de activar el piloto real de Hanoy (checkpoint de la mañana, abajo), Diego le escribió un "Hola" a su número real y el bot nunca respondió.

**1. Diagnóstico — el mensaje nunca llegó a `bot-handler`:** confirmado con SQL directo: `message_logs` tenía el inbound "Hola" (`status: received`, `meta-webhook` respondió 200 a Meta), pero CERO fila en `bot_message_debounce`, CERO entrada nueva en `bot_conversation_logs`, CERO respuesta saliente. Los logs de Edge Functions no mostraban ni un solo intento (éxito o 401) de invocar `bot-debounce-processor` cerca de esa hora — comparado con 2 invocaciones exitosas del Demo Bot ~1h antes (mismo código, distinta org), lo que descartó que fuera algo específico de la línea de Hanoy.

**2. Causa real — condición de carrera en el despacho fire-and-forget:** `enqueueDebounceMessage` (`meta-webhook/index.ts`) dispara un `fetch()` SIN `await` hacia `bot-debounce-processor` justo antes de que el handler retorne su respuesta 200 a Meta. Sin `EdgeRuntime.waitUntil()`, el runtime de Supabase puede reciclar la instancia apenas se envía esa respuesta, cortando el fetch en pleno vuelo sin dejar ningún rastro en logs (ni siquiera un intento fallido) — exactamente lo observado. Explica también por qué el Demo Bot "sí funcionaba": es una carrera, no un fallo determinístico — a veces el fetch alcanza a salir antes de que se recicle la instancia, a veces no. **Fix aplicado:** el `fetch()` ahora se envuelve en `(globalThis as any).EdgeRuntime?.waitUntil(dispatchPromise)`. Deployado (`npx supabase functions deploy meta-webhook --no-verify-jwt`).

**3. Hallazgo paralelo — cron de respaldo roto desde que se creó:** el cron `bot-debounce-sweep-stuck` (jobid 13, `* * * * *`, pensado como red de seguridad para ráfagas abandonadas) manda headers `apikey`/`Authorization: Bearer <anon key>`, pero `bot-debounce-processor` exige un header `x-internal-secret` propio (`INTERNAL_FUNCTION_SECRET` o `BENCH_TRIGGER_TOKEN`) — **401 en las 725 corridas desde que se creó esa misma mañana (04:49 UTC), cero éxitos**. Significa que si el despacho directo falla por cualquier razón, nada rescata el mensaje — se pierde en silencio para siempre. **FIX PENDIENTE:** no se pudo completar porque el valor de `INTERNAL_FUNCTION_SECRET` no es legible vía SQL ni MCP (vive solo en el runtime de Edge Functions, no en Vault ni en el repo) — Diego debe proporcionar el valor real (para armar `cron.alter_job(13, command := ...)` con el header correcto) o decidir rotarlo (`npx supabase secrets set INTERNAL_FUNCTION_SECRET=<nuevo>` + redeploy de TODAS las funciones que lo usan: `meta-webhook`, `bot-handler`, `bot-debounce-processor`, `messaging-gateway`, `process-media-async`, y otras — ver grep de `INTERNAL_FUNCTION_SECRET` en `supabase/functions/`).

**4. Hallazgo de Diego (fuera de código) — sin método de pago en Meta:** revisando la cuenta de WhatsApp Business de Hanoy, Diego encontró que no tenía método de pago registrado — se lo pidió agregar. **No se aisló al 100%** si esto o la condición de carrera (o ambas) causaron el silencio específico del 27 Jul — son causas independientes que pudieron coincidir. Contexto adicional relevante: Hanoy usa una tarjeta que solo carga fondos cuando quiere correr ads en Meta, dejando la factura en mora el resto del tiempo — esto es un **riesgo recurrente**, no un evento aislado (documentado en memoria persistente `project_hanoy-billing-riesgo-bot-silencio.md`, fuera del repo). Próxima vez que "el bot no responde" se reporte para Hanoy, revisar el método de pago de Meta como sospechoso de primera línea, junto con (no en vez de) el diagnóstico de logs habitual.

**5. Ajuste de copy — saludo genérico, sin preguntar tratamiento:** Diego probó el saludo del SDR en el Demo Bot y no le gustó que preguntara directo "¿en qué tratamiento está interesado?" — pidió algo más genérico tipo "Hola, gracias por escribir a la clínica X, ¿en qué le puedo ayudar?", con X = nombre de la clínica configurado o, si no hay uno distintivo, "del Dr. [nombre]"/"de la Dra. [nombre]" usando el identificador de género ya existente en la plataforma (`doctors.prefix`).
- **Implementado en `_shared/sdr-prompt.ts`:** nuevo campo `clinicDisplayName` en `SdrContext` (poblado en `buildSdrContext` desde `clinics.name` de la org, con fallback a `organizations.name`); nueva función `clinicIdentityPhrase(rawName)` que detecta si el nombre configurado arranca con "Dr"/"Dra" (regex, tolera con/sin punto, con/sin espacio) y lo reescribe como "del Dr. X"/"de la Dra. X" — si NO arranca así (marca propia real, ej. "OrionCare"), lo deja tal cual. Se aplicó tanto en la línea de intro del prompt ("Sos la asistente virtual de la clínica {X}...") como en la instrucción del guión de saludo (con ejemplo textual explícito para que el LLM copie el patrón, en vez de dejarlo a su criterio).
- Decisión de diseño: la identidad se calcula UNA VEZ del nombre ya configurado (clínica u org), no del roster de doctores de la org — evita el caso hub/silla-compartida (orgs como la de Hanoy tienen 2+ filas en `doctors`, con lo cual "elegir un doctor" del roster habría sido ambiguo).
- **Verificación:** `deno check` limpio en `sdr-prompt.ts` y `bot-handler/index.ts`; `sdr-guards.test.ts` 17/17 sin regresión. Deployado (`npx supabase functions deploy bot-handler --no-verify-jwt`). Diego probó 2 veces reseteando la sesión de prueba del Demo Bot (`UPDATE bot_sessions`, nunca `DELETE`) pero **no confirmó explícitamente el resultado** antes de cerrar la sesión — pendiente verificar en la próxima.

**Verificación pendiente para la próxima sesión:**
1. Esperar a que Hanoy agregue el método de pago en Meta, luego mandar un mensaje real a su línea para confirmar que el bot responde (valida en conjunto el fix de pago + el fix de código).
2. Completar el fix del cron `bot-debounce-sweep-stuck` (jobid 13) — necesita el valor real de `INTERNAL_FUNCTION_SECRET` de Diego, o su decisión de rotarlo.
3. Confirmar con Diego si el saludo nuevo lee bien en la práctica (no llegó confirmación explícita esta sesión).

---

## ▼ CHECKPOINT 27 Jul 2026 — QA en vivo del SDR (Demo Bot) → 6 fixes + activación real Hanoy

**Contexto:** Diego duplicó el catálogo (10 servicios dentales) y las 5 FAQs reales de Hanoy en la org demo (OrionCare, `sdr_mode_enabled=true`) para poder mandarle mensajes reales por WhatsApp "como si fuera paciente" y ver las respuestas del SDR sin arriesgar producción. De ahí salieron 6 bugs reales, todos corregidos y desplegados en el mismo `bot-handler` (afecta a cualquier org con SDR, no solo el demo).

**1. Media sin texto (`bot-handler/index.ts`):** el guard de "mensaje vacío" (sticker/audio/imagen sin transcribir) mandaba "🤔 Solo veo un audio/imagen/sticker...". Diego: no se siente natural. Cambiado a `message: ''` — `routeToBotHandler` (`_shared/bot-routing.ts:50`) ya omite el envío cuando el mensaje viene vacío, así que el bot queda en silencio real sin avanzar el estado. **Aplica a TODOS los clientes**, no solo SDR (el guard es universal, anterior al switch de estados).

**2. Confirmación directa con día+hora exactos (`bot-handler/sdr.ts::offerSlotsForDate`):** pedir "para el sábado a las 10?" devolvía un menú de 3 opciones (9:30/10:00/10:30) que ya incluía la hora pedida, en vez de confirmar directo. Fix: si `preferredTime` está libre y el día no se movió, `acceptSlot()` directo.

**3. `parseDateHint` no reconocía "fin de semana" (`bot-handler/index.ts`):** caía siempre al fallback genérico ("mañana, el viernes, o dígame la fecha"). Agregada regla: resuelve al sábado más próximo (clínicas hondureñas no suelen trabajar domingo); si no hay cupo, el escaneo hacia adelante que ya existía en `offerSlotsForDate` cubre domingo/lunes.

**4. Pregunta informativa sobre cita existente disparaba el menú de reagendar (`bot-handler/sdr.ts::handleSdrChat`):** "para cuándo es mi cita?" con intent `gestion_cita` sin acción clara (ni confirmar/cancelar/reagendar) caía al fallback clásico `handleGreeting`, cuyo detector `detectMenuIntent` (index.ts, distinto al `detectIntent` de honduras-intents) tiene la frase literal "cuando es mi cita" bucketeada junto con "reagendar"/"cancelar" — abría el menú de gestión en vez de solo informar. Fix: cuando hay cita existente y sin acción clara, se usa directo el `reply` que el LLM ya redactó (informado por `existingApptBlock`), sin caer al clásico.

**5. Rechazar la cita ofrecida + pedir otro día en `booking_confirm` caía a "Opción no válida" (`bot-handler/sdr.ts::maybeHandleSdr`):** "mejor no, para el próximo lunes tiene espacio?" no matcheaba `classicMatch` ni `detectIntent` (confirm/cancel), caía a `handleSdrConfirmQuestion` — que solo sabe "responder y re-preguntar por LA MISMA cita pendiente", sin horarios reales del nuevo día. Si el LLM inventaba una hora para el lunes, el guard la bloqueaba → `null` → clásico con texto crudo → "Opción no válida", sin salida real. Fix: nueva rama que detecta `parseDateHint` en el mensaje antes de caer a `handleSdrConfirmQuestion` — limpia el booking y re-ofrece slots reales del día nuevo vía `offerSlotsForDate`.

**6. Guard de precios no ataba el precio al servicio correcto + ambigüedad "si"/"sí":**
- `checkPriceGuard` solo validaba "¿es un precio real de ALGÚN servicio del catálogo?", no si es el precio del servicio del que se está hablando — un precio real pero mal atribuido (ej. confundir Evaluación con Blanqueamiento) pasaría igual. Nuevo helper `allowedPricesFor(ctx, serviceId)` en `sdr.ts` acota al precio del servicio en foco (booking en curso / cita existente) en los 5 puntos donde antes se usaba `publicPrices(ctx)` a secas; sin servicio identificado (chat general tipo "qué servicios ofrecen") se mantiene el catálogo completo.
- "si" sin tilde (condicional, "si tiene espacio el jueves") se confundía con "sí" afirmativo — WhatsApp casi nunca tilda y `normalizeTypos` ya quita acentos, así que quedan indistinguibles como texto. Nuevo `isConditionalSi()` (exportado desde `_shared/honduras-intents.ts`, reusado en el atajo local de `sdr.ts::maybeHandleSdr`) excluye el match de "si" cuando va seguido de un verbo hipotético (tiene, hay, puede, tuviera...). Verificado con casos manuales — "si" y "sí" solos (afirmación real) siguen funcionando.

**Deuda preexistente NO introducida por esta sesión:** 4 tests de `honduras-intents.test.ts` (soft_no/confirmo, faq_my_appointment, greeting solo, un caso Yeni/CF) ya fallaban antes de tocar el archivo — confirmado con `git stash` (mismo 25 passed/4 failed con y sin mis cambios). No se tocaron, quedan pendientes de revisión aparte.

**Verificación:** `deno check` limpio en los 3 archivos tras cada cambio; `sdr-guards.test.ts` 17/17 y `honduras-intents.test.ts` 25/29 (mismo baseline pre-existente) tras cada deploy; cada fix se probó en vivo contra el Demo Bot real (reset de sesión vía `UPDATE bot_sessions`, nunca `DELETE` — regla de la sesión del 24 Jul) antes de pasar al siguiente. Cada uno de los 6 fixes se desplegó individualmente con `npx supabase functions deploy bot-handler --no-verify-jwt` apenas se implementó (no se dejó código sin desplegar) — la ÚNICA vez que se olvidó desplegar antes de probar, el bug viejo reapareció en el chat, señal clara de que "editar el archivo" y "estar en producción" son pasos distintos con este flujo de deploy manual (no hay CI/CD en este repo para edge functions).

**Activación real:** con el OK explícito de Diego ("daremos como aprobado, lo vamos a tirar a campo"), se prendió `bot_enabled=true` + `sdr_mode_enabled=true` en la línea real de Dra Hanoy Medina (`whatsapp_lines.id=5303882f...`, número `+504 8984-1891`). Sin sesiones vivas que limpiar (las 4 existentes ya estaban expiradas). Antes de activar se resolvió el hallazgo de la sesión anterior (Limpieza Dental / Prótesis Total con `requires_prior_consult=false` y sin precio — rompía la premisa "cero excepciones" documentada en `sdr-prompt.ts:81-88`): Diego le cargó precio real (L1100) a Limpieza Dental; Prótesis Total se deja tal cual (decisión explícita, aceptada). Se corrigió también una idea equivocada sobre `whatsapp_lines.bot_greeting`: ese campo NO se usa en modo SDR (solo lo lee el flujo clásico `handleGreeting`) — el saludo lo improvisa el LLM cada vez a partir de `organizations.name`; no bloqueaba la activación.

**Pendiente / próxima sesión:** semana de supervisión del piloto (revisar respuestas reales + `llm_call_logs`), decidir si el Demo Bot sigue con el catálogo de Hanoy duplicado o se revierte a los servicios genéricos, y considerar si vale la pena automatizar el deploy de edge functions (hoy 100% manual vía CLI local) dado que se olvidó una vez en esta misma sesión.

---

## ▼ CHECKPOINT 24 Jul 2026 PM — Bot SDR híbrido: Fases 0-2b completas + QA en vivo

**Contexto de memoria:** el detalle completo del proyecto vive en la memoria persistente `project_bot-sdr-hibrido.md` (fuera del repo). Este checkpoint es el resumen operativo para retomar.

**1. Fix 12h (commit `7a49dcc`):** `PatientDetail`, `AppointmentsReport` (tabla+card), `DoctorScheduleExceptionsPage`, toast de conflicto de `NewScheduleExceptionModal` — todo a 12h vía `formatTimeTo12Hour`/`'h:mm a'`. CSV export queda en 24h a propósito. `AppointmentRow.tsx` es huérfano (0 importadores) — candidato a borrar.

**2. Humanización — medición pendiente desde mayo, corrida hoy:** meta >50% SUPERADA solo con Sprint 1 (68% agregado clientes reales, Wilmer 0%→80%, inválidas 22%→4%). Bitácora en `.claude/plans/bot-humanizacion.md`. Sprints 2-3 viejos = obsoletos, absorbidos por el SDR.

**3. Bot SDR híbrido — TODO el proyecto en `project_bot-sdr-hibrido.md`. Lo esencial:**
- **Origen:** Hanoy, 102 conversaciones/2.5 días de sus ads, contesta a mano, 4% conversión. Fase 0 con Diego: LLM entiende/redacta, máquina de estados decide; precios flag-por-servicio; follow-up→v2 (solo etiqueta `seguimiento`); FAQs las aprueba admin org; benchmark decidió modelo.
- **Fase 1 (BD, aplicada):** `service_types.price_is_public`, `whatsapp_lines.sdr_mode_enabled`, `conversations.lead_stage`(+interés+ts+índice), `bot_faq_proposals`+`approve_faq_proposal()` (INVOKER, códigos PROPOSAL_*), `llm_call_logs`, `organizations.llm_monthly_budget_usd` (default $10). Migraciones `20260724120000` y `20260724180000`.
- **Fase 2:** `_shared/llm.ts` (3 proveedores, no-throw, MODEL_PRICING, keys validadas vía función `llm-ping`), `_shared/sdr-prompt.ts` (prompt 100% desde BD + guión Hanoy + parser tolerante), `llm-benchmark` (25 casos reales etiquetados). **Resultado: Gemini 3.5 Flash-Lite 96%/0 fallos JSON/758ms/$2.61-mes >> Haiku 4.5 (fallback) >> GPT-4o-mini DESCARTADO (36% JSON roto)**. Reporte: `docs/reporte-benchmark-llm-bot-sdr-24jul.md`. OJO: Gemini 3.x Flash full salió MÁS caro que Haiku — el barato es flash-lite. Gemini en plan Paid confirmado por Diego ($10).
- **Fase 2b (integración):** `bot-handler/sdr.ts` con inyección de deps (cero circular); estados `sdr_chat`/`sdr_booking`; inerte con flag off; booking conversacional (LLM extrae `date_text` textual → `parseDateHint` backend → slots REALES → oferta 2-3 → guards `sdr-guards.ts` 17 tests deno → confirm clásico intacto). Fallbacks a menú clásico: LLM caído (Gemini→Haiku→clásico), JSON inválido, presupuesto (cache 10min). Embudo `lead_stage` desde backend.
- **Bugs cazados y arreglados:** (a) `.neq('agendado')` excluía filas NULL (lógica trivalente) → `.or(is.null,neq)`; (b) "Está bien"/preguntas en `booking_confirm` caían a "Opción no válida" → `detectIntent` + LLM re-pregunta; (c) "Gracias!" post-cita re-disparaba booking por historial contaminado → limpieza total al revivir `completed` + nota sintética para despedida cálida + intents pasivos jamás disparan booking.

**4. Config aplicada en orgs (BD prod):**
- Hanoy: `price_is_public=true` en Evaluación L460 + Blanqueamiento L3800 (precios que ELLA ya cotiza — L3800 estaba en su catálogo desde la instalación, Diego lo confirmó); 5 FAQs creadas con sus respuestas TEXTUALES del inbox (ubicación Torre Morazán, horarios desde 9AM, niños sí, cita previa, blanqueamiento 1 cita). Su bot sigue OFF.
- Demo Bot (OrionCare): `sdr_mode_enabled=TRUE` (QA de Diego en curso), Consulta general L500 pública. OJO: sus FAQs demo tienen precios que CONTRADICEN el catálogo (FAQ L1,200 vs catálogo L500) — el guard bloquea, pero es data demo sucia. **Regla nueva para pilotos: FAQs sin precios, los precios viven solo en el catálogo.**

**5. Notas técnicas para la próxima sesión:**
- QA sin WhatsApp: `bot-handler` acepta `BENCH_TRIGGER_TOKEN` como segunda credencial interna (harness: POST directo con el payload de meta-webhook; el bot no envía mensajes, eso lo hace el caller). Token en Supabase secrets; hay copia de esta sesión en scratchpad (efímera — si se pierde, `npx supabase secrets set BENCH_TRIGGER_TOKEN=<nuevo>`). Flagueado a Diego, aceptado.
- Deno local: NO instalado; usar `npx --yes deno-bin <cmd> --no-lock` (el deno.lock del repo es v5, incompatible con deno-bin viejo — por eso `--no-lock`).
- Encoding: mandar tildes vía bash/curl al harness se mojibakea — para probar frases con tilde usar el archivo JSON o WhatsApp real (de WhatsApp llegan limpias).
- **Al prender `sdr_mode_enabled` en una línea: borrar las `bot_sessions` vivas de esa línea** — el flag se cachea por sesión (TTL 30min). Ya mordió a Diego en su QA.
- La cita de prueba de Diego (lunes 27 Jul, 11:00 AM, Dr. Diego Escalante, org demo) quedó real en su agenda — borrarla o dejarla es su call.

**5b. Segunda mitad de sesión (post cambio Fable→Sonnet) — hardening de fuente de verdad:**
- Diego notó que el bot citaba una FAQ de horario ("lunes a viernes") que contradecía el horario real, y pidió estructurar esto: si hay tabla real, el SDR debe usarla y las FAQs de ese dominio deben ignorarse (para que un médico que cambia su horario y olvida actualizar la FAQ no rompa nada).
- **Auditoría de TODAS las orgs** (no solo demo/Hanoy) confirmó 3 dominios de riesgo real: horario (Wilmer/Hanoy/Ecoclinicas/Medilaser/OrionCare todos duplican; Ecoclinicas tiene 2 FAQs de horario CONTRADICTORIAS entre sí, bug ya viejo), precio (guard reactivo ya existía, ahora también filtro preventivo), especialidad/nombres de doctor (Medilaser hardcodea 3 nombres de dermatólogas que ya están en `doctors`+`specialties`). Ubicación/pagos/políticas generales confirmado que NO tienen tabla real poblada en la práctica (`clinics.address` casi siempre NULL) — siguen siendo FAQ legítima.
- **Implementado en `_shared/sdr-prompt.ts`:** `filterStructuredDomainFaqs()` (regex determinístico por keywords horario/precio/especialidad, excluye del prompt, logea qué excluyó — auditable); `SdrContext.doctors[]` nuevo con horario real + especialidad por profesional; bloque `## Horario real de atención` inyectado en el prompt ANTES de las FAQs, con instrucción explícita de que gana sobre cualquier FAQ; prompt reforzado para que preguntas de disponibilidad de UN DÍA CONCRETO ("¿tiene cupo el jueves?") siempre enruten a `sdr_booking` (motor real, incluye excepciones/bloqueos) — solo preguntas GENERALES de horario usan el resumen semanal.
- **Hallazgo de arquitectura importante:** el horario real NO vive solo en `doctor_schedules` — el motor consulta `calendar_schedules` PRIMERO (`_shared/availability.ts::loadSchedulesAllDows`, fallback a `doctor_schedules`). Mi primer intento leía solo `doctor_schedules` y habría introducido el MISMO bug (demo: `doctor_schedules` decía lunes-martes, `calendar_schedules` real decía lunes-sábado para el Dr. Escalante). Corregido reusando `loadSchedulesAllDows()` tal cual — nunca reinventar la query de horarios.
- **Bug adicional cazado (no relacionado a FAQs):** preguntar disponibilidad de un día SIN mencionar tratamiento antes, en una org con 2+ servicios, hacía que el bot cayera en silencio (repetía la respuesta del LLM sin agendar nada — callejón sin salida real). Fix: pregunta explícita por el tratamiento antes de entrar a `sdr_booking`.
- **Lección operativa grave:** `bot_sessions` → `bot_conversation_logs` es `ON DELETE CASCADE`. Al hacer `DELETE FROM bot_sessions` para destrabar la sesión de Diego (dos veces), se borró el historial completo de su conversación de prueba real — no se pudo recuperar el texto exacto de sus mensajes sobre sábado/jueves. **Regla nueva: usar `UPDATE` para resetear estado de sesión, nunca `DELETE`, salvo sesiones de prueba propias sin valor de evidencia.**
- Todo verificado end-to-end con harness directo (función `sdr-debug` temporal, creada y BORRADA en la misma sesión tras usarla para inspeccionar el JSON crudo del LLM). Deno tests de guards siguen en 17/17. `tsc` limpio. Commit separado del resto de Fase 2b.

**6. PENDIENTE / PRÓXIMA SESIÓN:**
1. **Diego termina su ronda de QA por WhatsApp en Demo Bot** (SDR está ON ahí). Recoger feedback, decidir si se apaga o queda para demos de venta.
2. **Fase 3 — UI:** switch "precio público" en config de servicios (MotorConfigPanel), pantalla de propuestas FAQ (`bot_faq_proposals` — tabla existe pero NADA la puebla aún: el pipeline de detección de gaps es parte de Fase 3), filtro de leads por `lead_stage` en inbox.
3. **Fase 4:** RLS con usuarios reales (admin/secretary/doctor) para `bot_faq_proposals`; checklist piloto Hanoy: acuerdo "hasta X conversaciones/mes" (X y tier superior = decisión estrategia pendiente), validar FAQs/precios con ella, prender `bot_enabled`+`sdr_mode_enabled`+borrar sesiones vivas, semana supervisada revisando cada respuesta + `llm_call_logs` semanal.
4. Rollout post-piloto: Wilmer/Yeni (decisión Diego: TODAS las líneas con bot, secuenciado tras semana 1 de datos de Hanoy).

---

## ▼ CHECKPOINT 24 Jul 2026 — Calendario: rename + FAB Nueva cita en Drawer + swipe + limpieza

**Contexto:** continuación directa de la vista mensual entregada el 23 Jul. Diego usó la pantalla y pidió, en orden: (1) aprovechar mejor el espacio mobile con un botón "+" para crear cita desde el calendario + swipe entre meses + cuadros más grandes; (2) que el "+" abra un Drawer embebido en vez de navegar a otra pantalla; varios fixes en vivo sobre ese Drawer (teclado, sensibilidad de cierre, flash del buscador de profesional, auto-close al agendar, fecha de hoy preseleccionada); (3) renombrar la pantalla (ya no es "semanal"); (4) un pase de limpieza general sobre todo lo hecho.

**1. Espacio + swipe + FAB (`MonthGrid.tsx`, `Calendario.tsx`):**
- `MonthGrid.tsx`: 3 props nuevos retrocompatibles — `headerExtra?: ReactNode` (nodo al final del header no-colapsado, usado para fusionar el botón "Hoy" en la misma fila que las flechas ‹›, sacándolo de una fila propia); `swipeable?: boolean` (listeners nativos `onTouchStart/Move/End` con `useRef` para trackear posición, umbral 50px + filtro de ángulo `|deltaY| < |deltaX|*0.5` para no pelear con el scroll vertical del contenedor padre, guard de `isLoading`/`swipeable` al inicio de cada handler, sin `preventDefault` para no romper el scroll nativo); `largeDays?: boolean` (cuadros `h-20 text-lg` vs `h-10 text-sm` default, pedido explícito "al menos el doble"). Precedente de FAB copiado literal de `PromotionsPage.tsx:361-369` (`fixed bottom-6 right-6 h-14 w-14 rounded-full shadow-lg z-40`).
- Investigado con Explore agents: no había librería de gestos instalada (`embla-carousel-react` solo se usa en `ui/carousel.tsx`, paradigma distinto); implementar swipe a mano fue la opción correcta, sin overflow-x que interfiriera.

**2. FAB abre Drawer embebido, no navega (pedido explícito tras probar la v1 con `navigate`):**
- Extracción completa de `src/pages/NuevaCita.tsx` (742 líneas) a `src/components/NuevaCitaComposer.tsx` (helpers, sub-componentes `ServicePicker`/`DateTimePanel`/`MobileDateTime`/`ProfesionalSummary`, todo el cuerpo del composer) — firma nueva `NuevaCitaComposer({ initialDate?, onCreated? })`. `NuevaCita.tsx` quedó como wrapper de 15 líneas (`<MainLayout><NuevaCitaComposer .../></MainLayout>`, lee `location.state?.date`).
- `useAppointmentComposer.ts` ganó parámetro `initialDate?: Date` (inicializa `monthAnchor` y `selectedDate`).
- `Calendario.tsx`: `Drawer` (vaul) con `DrawerHeader` (título + X, patrón calcado de `NewScheduleExceptionModal.tsx`), `NuevaCitaComposer` adentro. `composerDate` (día tocado en el grid si sigue siendo del mes visible, si no `undefined`) se pasa como `initialDate`.

**3. Bugs reales de `vaul` — investigados leyendo `node_modules/vaul/dist/index.mjs` directamente, no supuestos:**
- **Botón "Agendar cita" se movía con el teclado:** causa real, `repositionInputs` (default `true` en `Drawer.Root`) redimensiona el drawer en vivo vía JS (`onVisualViewportChange`, escucha `visualViewport.resize`) cuando detecta un input enfocado — pensado para bottom-sheets cortos que necesitan crecer hacia el teclado, rompía nuestro form de 100dvh con footer fijo. Fix: `repositionInputs={false}`. (De paso también se agregó `interactive-widget=resizes-content` al viewport de `index.html` — corrige a nivel navegador que `dvh` no se recalculaba con el teclado abierto; requiere reload completo de la pestaña para tomar efecto, no solo SPA nav.)
- **Cierre por swipe "muy sensible", inconsistente:** subir `closeThreshold` (distancia, default 0.25) a 0.5 NO alcanzó — reproducido en vivo con Chrome (drag simulado): vaul cierra por distancia **O** por velocidad (`VELOCITY_THRESHOLD=0.4`, constante interna, no expuesta como prop), y un swipe rápido cierra con mínimo arrastre sin importar `closeThreshold`. Sin forma de afinar la velocidad, se optó (confirmado con Diego vía pregunta) por `dismissible={false}` — cierre 100% predecible solo con la X. Verificado en vivo: drags rápidos y lentos, cortos y largos, ya no cierran; X sigue funcionando.
- **Drawer anidado (`MobileDateTime` de fecha/hora, DENTRO del Drawer nuevo cuando se abre desde el FAB):** riesgo real encontrado en el código de vaul, no solo teórico — `usePositionFixed` (scroll-lock de body en iOS Safari) usa una variable global compartida entre TODOS los `Drawer.Root` de la página; sin la prop `nested` en el drawer interior, cerrarlo restauraría el scroll del body aunque el exterior siga abierto. Fix: `nested` en el `<Drawer>` de `MobileDateTime` (dentro de `NuevaCitaComposer.tsx`).

**4. Flash del buscador de profesional (~medio segundo) al abrir "Nueva cita":**
- Causa: `catalog` (servicios del org) arranca en `[]` mientras el fetch async resuelve, indistinguible de "el org no tiene servicios" — `path` se derivaba a `'duration'` durante la carga, que para orgs multi-doctor muestra el buscador de profesional en vez de "Servicios a realizar". Fix: flag nuevo `isLoadingCatalog` en `useAppointmentComposer.ts` (arranca `true`, `.finally()` lo baja), usado para gatear `requiresDoctorSelection` y para mostrar un spinner en vez de decidir la sección prematuramente en `NuevaCitaComposer.tsx`.

**5. Auto-close + fecha de hoy por default (pedidos post-prueba):**
- `onCreated` (nuevo, se dispara en `handleAgendar` después de `c.reset()` en el path exitoso) ahora en `Calendario.tsx` hace `refetch(); setNewCitaOpen(false);` — antes solo refrescaba, dejando el drawer abierto.
- "Hoy" preseleccionado: NO se resolvió en `composerDate` (lógica específica del FAB) sino en el hook (`useAppointmentComposer`: `selectedDate` inicial `initialDate ?? getLocalToday()`) — así aplica también a la página completa `/citas/nueva`, no solo al drawer. `composerDate` en `Calendario.tsx` quedó simple: día tocado si es del mes visible, si no `undefined` (deja que el hook default-seleccione hoy).

**6. Rename `AgendaSemanal` → `Calendario`:** alcance completo (archivo + ruta `/agenda-semanal`→`/calendario` + ~20 referencias) decidido con Diego vía pregunta — la alternativa de solo cambiar el label visible se descartó a favor de no dejar deuda. Nombre "Calendario" elegido para no confundir con "Agenda de Hoy" (pantalla aparte).

**7. Pase de limpieza (`/simplify`, 4 agentes en paralelo — reuso/simplificación/eficiencia/altitud) sobre el diff completo de la sesión:**
- Aplicado: `useWeeklyAgenda.ts` borrado (125 líneas, cero importadores tras el rename); ternario de 4 niveles para la clase del día en `MonthGrid.tsx` aplanado a función `dayVariantClass()`; guard `disabled={!canGoPrev || isLoading}` agregado al botón "mes anterior" (antes asimétrico, solo "siguiente" lo tenía); filtro siempre-`true` en `useMonthlyAgenda.ts` (`daysWithAppointments`) simplificado; condición de carrera real agregada a `useMonthlyAgenda.fetchAgenda` (mismo patrón request-id que ya usa `useAppointmentComposer.ts` — sin esto, un swipe rápido entre meses podía dejar pisado el resultado con el mes viejo); guard de montaje `{newCitaOpen && <NuevaCitaComposer/>}` en `Calendario.tsx` removido (redundante — Radix/vaul ya desmonta vía `Presence` al cerrar — y causaba un glitch visual de contenido en blanco a mitad de la animación de cierre). **Verificado en vivo con Chrome** que quitar el guard no rompe el reset de estado (abrir → escribir en buscador → cerrar → reabrir → form limpio, confirmado con `data-vaul-drawer` y lectura directa del `value` del input).
- Identificado pero NO aplicado (fuera del diff de esta sesión, riesgo/esfuerzo no proporcional a un cleanup): `NewScheduleExceptionModal.tsx` (bloqueador de horario) tiene el mismo patrón de Drawer full-screen con un input de texto ("Motivo") y NUNCA recibió `repositionInputs`/`dismissible` — mismo bug esperable, no reportado aún porque es de la sesión anterior. Formateo de hora 12h duplicado 4 veces en el repo (`dateUtils.formatTimeTo12Hour`, más copias `formatTimeParts`/`to12h` en `DayAppointmentsModal.tsx`/`AgendaMedico.tsx`/`NewScheduleExceptionModal.tsx`) y bloque de timeline de citas duplicado ~80 líneas entre `DayAppointmentsModal.tsx`/`AgendaMedico.tsx` — deuda preexistente heredada al mover código, no creada esta sesión.

**Verificación:** `tsc --noEmit` + `vite build` limpios después de cada cambio (no solo al final). Varios puntos verificados en vivo con Chrome real (no solo lectura de código): swipe con drags rápidos/lentos/cortos/largos tras `dismissible={false}`; reset de estado del composer tras sacar el guard de montaje; ruta `/calendario` y label "Calendario" en sidebar. Pendiente que Diego confirme en su celular real (mismo caveat de siempre — `resize_window` no funciona en este entorno para QA mobile, se usó el truco de forzar `isMobile=true` temporalmente, revertido en cada caso).

---

## ▼ CHECKPOINT 23 Jul 2026 — Vista mensual de agenda (Semana/Mes toggle + timeline compartido)

**Contexto (Fase 0):** Hanoy y UNIMED pidieron una vista mensual tipo TimeTree; `AgendaSemanal.tsx` (pantalla default de todos los roles) solo mostraba una tira de 7 días. Mockup de Stitch aprobado por Diego con 2 requisitos explícitos: el calendario mensual **nunca colapsa** (siempre visible para poder elegir día) y el detalle del día seleccionado se muestra inline debajo, en ambos modos y en mobile+desktop (mobile como foco de espacio). Plan completo en `.claude/plans/steady-discovering-lollipop.md`.

**Hallazgo clave que evitó tocar BD:** `get_weekly_agenda(p_user_id, p_week_start, p_week_end, p_doctor_id)` (verificado vía `pg_get_functiondef`) NO tiene nada hardcodeado a 7 días — es un rango de fechas genérico pese al nombre. Se pudo pedir el mes completo con la MISMA función expuesta (`getWeeklyAgenda()` en `src/lib/api.ts`), sin RPC nueva.

**Archivos:**
- `src/hooks/useMonthlyAgenda.ts` (nuevo) — hermano de `useWeeklyAgenda.ts`: recibe `{ userId, monthAnchor, doctorId, enabled }`, calcula el rango del mes con `startOfMonth`/`endOfMonth`, llama `getWeeklyAgenda()`, devuelve `{ doctors, appointmentsByDate, daysWithAppointments (Set), isLoading, refetch }`. Sin el prefetch de rangos adyacentes que sí tiene `useWeeklyAgenda` (rango ya es grande, no vale la pena).
- `src/components/MonthGrid.tsx` — 2 props nuevos retrocompatibles: `markedDays?: Set<string>` (punto bajo el día, blanco si el día está seleccionado) y `disablePast?: boolean` (default `true`, sin cambios para `NuevaCita`/bloqueador de horario; `false` en la vista mensual para poder ver agenda histórica).
- `src/pages/AgendaSemanal.tsx` — nuevo estado `viewMode: 'week'|'month'` con `ToggleGroup`; ambos hooks (`useWeeklyAgenda`/`useMonthlyAgenda`) montados siempre pero con `enabled: viewMode === X` para no pedir data del modo inactivo; en modo Mes se usa `MonthGrid` con `disablePast={false}`, `markedDays={daysWithAppointments}`, SIN `collapsible`; timeline de citas (antes solo tiempo+card) ahora tiene línea vertical + punto conectando cada hora, reusado en ambos modos — mismo `StatusBadge` + botón "Reagendar" (`RescheduleModal`) de siempre.

**Verificación:** SQL directo a `appointments` (org de prueba, julio 2026) confirmó que los puntos del grid mensual coinciden EXACTO con los días reales con citas — incluyendo que los domingos sin citas correctamente NO muestran punto (false positive descartado tras zoom del screenshot, que a baja resolución hacía parecer que todos los días tenían punto). Probado con rol Médico (vista forzada a 1 doctor) y Administrador (selector de doctor + auto-select single-doctor-org intactos). "Reagendar" desde el timeline en modo Mes abre `RescheduleModal` igual que en modo Semana (sin regresión). Mes sin citas (agosto 2026) muestra grid sin puntos, sin errores. `npx tsc --noEmit` y `npx vite build` limpios.

**Pendiente / fuera de alcance de esta sesión:** QA de viewport mobile real — `resize_window` de la automatización de browser sigue sin funcionar en este entorno (ver `reference_browser-resize-window-no-funciona.md`); a diferencia del modal del bloqueador de horario, esta pantalla NO usa `useIsMobile()` (es 100% CSS responsive con breakpoints `md:`), así que el truco de forzar el hook no sirve aquí — hace falta que Diego confirme en su celular. Vista combinada multi-doctor con color por doctor, layout de 2 columnas para desktop, y "hora actual en vivo" quedaron explícitamente fuera de alcance (ver plan).
...
### Seguimiento a leads fríos (backlogueado explícitamente, no tocar hasta que Diego lo traiga — 9 Ago)
- [ ] **43% de leads del SDR (3/7 desde su activación 27 Jul) quedan muertos sin ningún seguimiento** tras la primera respuesta del bot — ni el bot reintenta, ni un humano los ve a tiempo. Distinto del problema de tono (ver checkpoint 9 Ago "pregunta de interés") — acá nadie llegó a escribir nada más.
- [ ] **~34% de sesiones del bot clásico** (Wilmer 23%, Medilaser 42%, Consultorio Familiar 25%) mueren justo después de una respuesta de FAQ, sin elegir "1. Otra pregunta"/"2. Menú" — causa de fricción (hay que escribir un número), no de tono.
- Diego: "atenderemos primero a los más interesados" (la pregunta de interés) antes de meterle mano a esto — revisar cuando la primera parte muestre resultado.
