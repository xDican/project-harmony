...
> Ultima actualizacion: 3 Jun 2026 (PRIORIDAD #1 = Motor de Agendamiento Multi-Recurso. **FASE 0 + 1 + 2 + 3 ENTREGADAS; FASE 4 CASI CERRADA — solo falta el relabel "Doctor"→"Profesional" (#24).** Fase 4 entregado y **QA-APROBADO 3 Jun**: (a) booking service-first manual + enforcement, (b) servicios como fuente unica en `/admin/motor → Servicios`, (c) **vista combinada multi-profesional con auto-asignacion al menos cargado** (QA aprobado: capturas de Diego coinciden exacto con el motor, ver Fase 4 abajo). **PROXIMA SESION (cuando vuelva Diego): cerrar Fase 4 con el relabel #24** — decision pendiente: global "Profesional" (recomendado, [[codigo-agnostico-rubro]]) vs configurable por org. Day-view resource-aware queda deferido (cosmetico). Luego Fase 5 (secuenciador `visit_id`) o Fase 6 (bot service-first + skill). MCP Supabase disponible. Es el producto real, ver [[motor-agendamiento-es-producto]].)
...
| 4 | Vista combinada + booking service-first para Dulce + label Profesional | 6-8h | 🟢 **CASI CERRADA 3 Jun** (service-first + servicios fuente unica + vista combinada con auto-asignacion QA-aprobados; **solo falta relabel #24**; day-view deferido) |
...
**PENDIENTE para cerrar Fase 4: relabel "Doctor"→"Profesional" (#24).** Hoy la UI dice Medico/Doctor en varios lados (paso "Seleccionar Medico", "¿Con que doctor?", sidebar, etc.). El toast de exito de agendar YA dice "Profesional". Decision pendiente con Diego: global "Profesional" (recomendado por [[codigo-agnostico-rubro]]) vs configurable por org. Bajo esfuerzo/riesgo, es solo copy.
